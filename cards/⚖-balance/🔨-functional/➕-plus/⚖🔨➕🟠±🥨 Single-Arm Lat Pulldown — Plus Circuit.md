---
zip: ⚖🔨➕🟠
operator: 🥨 tendo
status: GENERATED
deck: 32
order: ⚖ Balance | ~70% | 10-12 reps | 90s | CNS: Moderate
axis: 🔨 Functional | Unilateral, standing, athletic-transfer movements.
type: ➕ Plus | Full body power, core
color: 🟠 Circuit | Tier 0-3 | GOLD: No | Station-based timed rotation. No barbells. Loop logic required.
blocks: ♨️ 🏗 🧈 🧩 🪫 🚂
---
# ➕ Single-Arm Lat Pulldown — Plus Circuit ➕

## Balance Functional — Plus focus (Circuit) · 40-50 min

**CODE:** ⚖🔨➕🟠

> "Find the weak link and spend time on it. Correction is the session."
═══
## 1) ♨️ Warm-Up — 🥨 tendo
Subcode: ⚖🔨➕🟠 (Warm-Up | Plus | Functional | Circuit)
├─ 12 ➕ Single-Leg Hops Through Ladder (station rotation, keep moving)
│  Set 1: ⚖ 50% × 12 (pattern prep)
Rest: 60s
═══
## 2) 🏗 Reformance
Subcode: ⚖🔨➕🟠 (Reformance | Plus | Functional | Circuit)
├─ 11 ➕ Half-Kneeling Cable Chop (corrective, address the weak link)
│  Set 1: ⚖ 50% × 13 (prehab)
│  Set 2: ⚖ 55% × 11 (stability)
Rest: 75s
═══
## 3) 🎱 ARAM — Circuit Loop
Subcode: ⚖🔨➕🟠 (ARAM | Plus | Functional | Circuit)
┌─ 3 rounds, rotate through stations. No two adjacent stations same muscle group.
│  Station 1: ➕ Single-Arm Lat Pulldown × 11 (station rotation, keep moving)
│  Station 2: ➕ Single-Leg Hops Through Ladder × 11 (station rotation, keep moving)
│  Station 3: ➕ Half-Kneeling Cable Chop × 11 (station rotation, keep moving)
│  Station 4: ➕ Single-Arm Face Pull × 11 (station rotation, keep moving)
│  Transition: 15s between stations
│  Round rest: 75s
Rest: 90s after final round
═══
## 5) 🪫 Release
Subcode: ⚖🔨➕🟠 (Release | Plus | Functional | Circuit)
├─ 12 ➕ Aquaman (Alternating Superman) (smooth tempo, downshift)
│  Set 1: ⚖ 50% × 12 (deload)
Rest: 75s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → ⚖🔨➕🟠 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
The correction is the progress. Symmetry before load, always.
