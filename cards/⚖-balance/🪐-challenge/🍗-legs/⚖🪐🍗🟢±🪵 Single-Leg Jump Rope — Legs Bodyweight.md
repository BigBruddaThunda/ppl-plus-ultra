---
zip: ⚖🪐🍗🟢
operator: 🪵 teneo
status: GENERATED
deck: 34
order: ⚖ Balance | ~70% | 10-12 reps | 90s | CNS: Moderate
axis: 🪐 Challenge | Hardest variation at any level. Deficit, pause, tempo, bands.
type: 🍗 Legs | Quads, hamstrings, glutes, calves
color: 🟢 Bodyweight | Tier 0-2 | GOLD: No | No gym required. Park, hotel, living room.
blocks: ♨️ 🏗 🧈 🧩 🪫 🚂
---
# 🍗 Single-Leg Hip Thrust (Barbell) — Legs Bodyweight 🍗

## Balance Challenge — Legs focus (Bodyweight) · 40-50 min

**CODE:** ⚖🪐🍗🟢

> "Find asymmetries with bodyweight. No equipment hides the gap."

*Climbing / Grip Sport: Bodyweight functional — park, field, living room — prove the pattern works anywhere*
═══
## 1) ♨️ Warm-Up — 🪵 teneo
Subcode: ⚖🪐🍗🟢 (Warm-Up | Legs | Challenge | Bodyweight)
├─ 12 🍗 Single-Leg Hip Thrust (Bodyweight) (bodyweight, no external load needed)
│  Set 1: ⚖ 50% × 12 (pattern prep)
Rest: 60s
═══
## 2) 🏗 Reformance
Subcode: ⚖🪐🍗🟢 (Reformance | Legs | Challenge | Bodyweight)
├─ 11 🍗 Single-Leg Glute Bridge (corrective, address the weak link)
│  Set 1: ⚖ 50% × 12 (prehab)
│  Set 2: ⚖ 55% × 11 (stability)
Rest: 75s
═══
## 3) 🧈 Bread & Butter
Subcode: ⚖🪐🍗🟢 (Bread & Butter | Legs | Challenge | Bodyweight)
├─ 11 🍗 Single-Leg Hip Thrust (Barbell) (bodyweight, no external load needed)
│  Set 1: ⚖ 65% × 11 (build set)
│  Set 2: ⚖ 70% × 11 (working set)
│  Set 3: ⚖ 70% × 11 (repeat quality)
Rest: 90s
═══
## 4) 🧩 Supplemental
Subcode: ⚖🪐🍗🟢 (Supplemental | Legs | Challenge | Bodyweight)
├─ 12 🍗 Single-Leg Balance Hold (full range, different angle)
│  Set 1: ⚖ 60% × 12 (support volume)
│  Set 2: ⚖ 60% × 12 (non-redundant)
Rest: 75s
═══
## 5) 🪫 Release
Subcode: ⚖🪐🍗🟢 (Release | Legs | Challenge | Bodyweight)
├─ 12 🍗 Single-Leg Balance with Perturbation (smooth tempo, downshift)
│  Set 1: ⚖ 50% × 12 (deload)
Rest: 75s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → ⚖🪐🍗🟢 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Bodyweight exposed the asymmetry. Note which side lagged.
