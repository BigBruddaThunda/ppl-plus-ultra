---
zip: ⚖🐬🛒🟡
operator: 🧲 capio
status: GENERATED
deck: 36
order: ⚖ Balance | ~70% | 10-12 reps | 90s | CNS: Moderate
axis: 🐬 Partner | Social training context. Spottable, alternating, teachable.
type: 🛒 Push | Chest, front delts, triceps
color: 🟡 Fun | Tier 0-5 | GOLD: No | Exploration and variety. Structured play within constraints.
blocks: ♨️ 🏗 🧈 🧩 🪫 🚂
---
# 🛒 Single-Arm Cable Rear Delt Fly — Push Fun 🛒

## Balance Partner — Push focus (Fun) · 40-50 min

**CODE:** ⚖🐬🛒🟡

> "Explore movements that challenge your weakest positions."

*Stress Management: Somatic exploration — movements you would not normally try, body awareness*
═══
## 1) ♨️ Warm-Up — 🧲 capio
Subcode: ⚖🐬🛒🟡 (Warm-Up | Push | Partner | Fun)
├─ 12 🛒 Single-Arm Overhead Dumbbell Hold (explore, stay within constraints)
│  Set 1: ⚖ 50% × 12 (pattern prep)
Rest: 60s
═══
## 2) 🧈 Bread & Butter
Subcode: ⚖🐬🛒🟡 (Bread & Butter | Push | Partner | Fun)
├─ 11 🛒 Single-Arm Cable Rear Delt Fly (explore, stay within constraints)
│  Set 1: ⚖ 65% × 11 (build set)
│  Set 2: ⚖ 70% × 11 (working set)
│  Set 3: ⚖ 70% × 11 (repeat quality)
Rest: 90s
═══
## 3) 🌎 Exposure
Subcode: ⚖🐬🛒🟡 (Exposure | Push | Partner | Fun)
├─ 11 🛒 Medicine Ball Chest Pass (try the unfamiliar version, explore the range)
│  Set 1: ⚖ 55% × 11 (discovery set)
Rest: 75s
═══
## 4) 🏖 Sandbox
Subcode: ⚖🐬🛒🟡 (Sandbox | Push | Partner | Fun)
├─ 11 🛒 Medicine Ball Slam (Chest Height) (explore within constraints, choose your variation)
│  Option A: ⚖ 60% × 11 (play with grip, stance, or tempo)
│  Option B: ⚖ 60% × 11 (try a variation you have not used before)
Rest: 75s
═══
## 5) 🪫 Release
Subcode: ⚖🐬🛒🟡 (Release | Push | Partner | Fun)
├─ 12 🛒 Medicine Ball Chest Pass (smooth tempo, downshift)
│  Set 1: ⚖ 50% × 12 (deload)
Rest: 75s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → ⚖🐬🛒🟡 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Exploring weak positions teaches the body new ranges. Track the discovery.
