---
zip: ⚖🏛🍗⚪
operator: 📍 pono
status: GENERATED
deck: 31
order: ⚖ Balance | ~70% | 10-12 reps | 90s | CNS: Moderate
axis: 🏛 Basics | Bilateral, stable, barbell-first classics. Compound over isolation.
type: 🍗 Legs | Quads, hamstrings, glutes, calves
color: ⚪ Mindful | Tier 0-3 | GOLD: No | Slow tempo (4s eccentrics). Extended rest (2+ min). Breathing cues.
blocks: ♨️ 🏗 🧈 🧩 🪫 🚂
---
# 🍗 Single-Leg Extension — Legs Mindful 🍗

## Balance Basics — Legs focus (Mindful) · 40-50 min

**CODE:** ⚖🏛🍗⚪

> "Find the weak link and spend time on it. Correction is the session."
═══
## 1) ♨️ Warm-Up — 📍 pono
Subcode: ⚖🏛🍗⚪ (Warm-Up | Legs | Basics | Mindful)
├─ 12 🍗 Deep Squat Hold (Ankle Mobility) (4s eccentric, breath-paced)
│  Set 1: ⚖ 50% × 12 (pattern prep)
Rest: 60s
═══
## 2) 🏗 Reformance
Subcode: ⚖🏛🍗⚪ (Reformance | Legs | Basics | Mindful)
├─ 11 🍗 Single-Leg Deadlift (Glute Med Stability) (corrective, address the weak link)
│  Set 1: ⚖ 50% × 13 (prehab)
│  Set 2: ⚖ 55% × 11 (stability)
Rest: 75s
═══
## 3) 🧈 Bread & Butter
Subcode: ⚖🏛🍗⚪ (Bread & Butter | Legs | Basics | Mindful)
├─ 11 🍗 Single-Leg Extension (4s eccentric, breath-paced)
│  Set 1: ⚖ 65% × 11 (build set)
│  Set 2: ⚖ 70% × 11 (working set)
│  Set 3: ⚖ 70% × 11 (repeat quality)
Rest: 90s
═══
## 4) 🧩 Supplemental
Subcode: ⚖🏛🍗⚪ (Supplemental | Legs | Basics | Mindful)
├─ 13 🍗 Back Squat (High Bar) (full range, different angle)
│  Set 1: ⚖ 60% × 13 (support volume)
│  Set 2: ⚖ 60% × 13 (non-redundant)
Rest: 75s
═══
## 5) 🪫 Release
Subcode: ⚖🏛🍗⚪ (Release | Legs | Basics | Mindful)
├─ 12 🍗 Walking Lunge (parasympathetic, tension down)
│  Set 1: ⚖ 50% × 12 (deload)
Rest: 75s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → ⚖🏛🍗⚪ — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
The correction is the progress. Symmetry before load, always.
