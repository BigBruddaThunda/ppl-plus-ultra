---
zip: ⚖🏛🍗⚫
operator: 📍 pono
status: GENERATED
deck: 31
order: ⚖ Balance | ~70% | 10-12 reps | 90s | CNS: Moderate
axis: 🏛 Basics | Bilateral, stable, barbell-first classics. Compound over isolation.
type: 🍗 Legs | Quads, hamstrings, glutes, calves
color: ⚫ Teaching | Tier 2-3 | GOLD: No | Extra rest, coaching cues, comprehension over exertion.
blocks: ♨️ 🏗 🧈 🧩 🪫 🚂
---
# 🍗 Lateral Lunge — Legs Teaching 🍗

## Balance Basics — Legs focus (Teaching) · 40-50 min

**CODE:** ⚖🏛🍗⚫

> "Find the weak link and spend time on it. Correction is the session."
═══
## 1) ♨️ Warm-Up — 📍 pono
Subcode: ⚖🏛🍗⚫ (Warm-Up | Legs | Basics | Teaching)
├─ 12 🍗 Deep Squat Hold (Ankle Mobility) (coached, check form before adding load)
│  Set 1: ⚖ 50% × 12 (pattern prep)
Rest: 60s
═══
## 2) 🏗 Reformance
Subcode: ⚖🏛🍗⚫ (Reformance | Legs | Basics | Teaching)
├─ 11 🍗 Back Squat (High Bar) (corrective, address the weak link)
│  Set 1: ⚖ 50% × 13 (prehab)
│  Set 2: ⚖ 55% × 11 (stability)
Rest: 75s
═══
## 3) 🧈 Bread & Butter
Subcode: ⚖🏛🍗⚫ (Bread & Butter | Legs | Basics | Teaching)
├─ 11 🍗 Lateral Lunge (coached, check form before adding load)
│  Set 1: ⚖ 65% × 11 (build set)
│  Set 2: ⚖ 70% × 11 (working set)
│  Set 3: ⚖ 70% × 11 (repeat quality)
Rest: 90s
═══
## 4) 🧩 Supplemental
Subcode: ⚖🏛🍗⚫ (Supplemental | Legs | Basics | Teaching)
├─ 13 🍗 Walking Lunge (full range, different angle)
│  Set 1: ⚖ 60% × 13 (support volume)
│  Set 2: ⚖ 60% × 13 (non-redundant)
Rest: 75s
═══
## 5) 🪫 Release
Subcode: ⚖🏛🍗⚫ (Release | Legs | Basics | Teaching)
├─ 12 🍗 Spanish Squat (Banded VMO) (smooth tempo, downshift)
│  Set 1: ⚖ 50% × 12 (deload)
Rest: 75s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → ⚖🏛🍗⚫ — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
The correction is the progress. Symmetry before load, always.
