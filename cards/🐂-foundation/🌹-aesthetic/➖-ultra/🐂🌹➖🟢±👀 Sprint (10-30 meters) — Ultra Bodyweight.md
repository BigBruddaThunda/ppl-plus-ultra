---
zip: 🐂🌹➖🟢
operator: 👀 specio
status: GENERATED
deck: 03
order: 🐂 Foundation | <=65% | 8-15 reps | 60-90s | CNS: Low
axis: 🌹 Aesthetic | Isolation, full ROM, mind-muscle connection. Feel over load.
type: ➖ Ultra | Cardiovascular system
color: 🟢 Bodyweight | Tier 0-2 | GOLD: No | No gym required. Park, hotel, living room.
blocks: ♨️ 🔢/🛠 🧈 🧩 🧬 🚂
---
# ➖ Sprint (10-30 meters) — Ultra Bodyweight ➖

## Foundation Aesthetic — Ultra focus (Bodyweight) · 40-50 min

**CODE:** 🐂🌹➖🟢

> "Learn the pattern with your own body first. External load comes later."
═══
## 1) ♨️ Warm-Up — 👀 specio
Subcode: 🐂🌹➖🟢 (Warm-Up | Ultra | Aesthetic | Bodyweight)
├─ 12 ➖ Kettlebell Goblet Squat (bodyweight, no external load needed)
│  Set 1: 🐂 45% × 12 (pattern prep)
Rest: 60s
═══
## 2) 🔢 Fundamentals
Subcode: 🐂🌹➖🟢 (Fundamentals | Ultra | Aesthetic | Bodyweight)
├─ 12 ➖ Sprint (10-30 meters) (slow, own each position)
│  Set 1: 🐂 45% × 12 (grounding)
│  Set 2: 🐂 50% × 12 (pattern lock)
Rest: 75s
═══
## 3) 🧈 Bread & Butter
Subcode: 🐂🌹➖🟢 (Bread & Butter | Ultra | Aesthetic | Bodyweight)
├─ 11 ➖ Sprint (10-30 meters) (bodyweight, no external load needed)
│  Set 1: 🐂 60% × 11 (build set)
│  Set 2: 🐂 65% × 11 (working set)
│  Set 3: 🐂 65% × 11 (repeat quality)
Rest: 75s
═══
## 4) 🧩 Supplemental
Subcode: 🐂🌹➖🟢 (Supplemental | Ultra | Aesthetic | Bodyweight)
├─ 13 ➖ Sprint (30-60 meters) (full range, different angle)
│  Set 1: 🐂 55% × 13 (support volume)
│  Set 2: 🐂 55% × 13 (non-redundant)
Rest: 60s
═══
## 5) 🧬 Imprint
Subcode: 🐂🌹➖🟢 (Imprint | Ultra | Aesthetic | Bodyweight)
├─ 15 ➖ Sprint (10-30 meters) (high rep, low load, neural memory)
│  Set 1: 🐂 40% × 15 (lock the pattern)
Rest: 60s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🐂🌹➖🟢 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
If the pattern held at bodyweight, it will hold under load. Graduate when ready.
