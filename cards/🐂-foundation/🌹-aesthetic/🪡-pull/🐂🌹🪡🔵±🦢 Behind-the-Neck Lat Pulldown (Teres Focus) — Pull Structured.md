---
zip: 🐂🌹🪡🔵
operator: 🦢 plico
status: GENERATED
deck: 03
order: 🐂 Foundation | <=65% | 8-15 reps | 60-90s | CNS: Low
axis: 🌹 Aesthetic | Isolation, full ROM, mind-muscle connection. Feel over load.
type: 🪡 Pull | Lats, rear delts, biceps, traps, erectors
color: 🔵 Structured | Tier 2-3 | GOLD: No | Prescribed sets/reps/rest. Trackable and repeatable.
blocks: ♨️ 🔢/🛠 🧈 🧩 🧬 🚂
---
# 🪡 Archer Pull-Up — Pull Structured 🪡

## Foundation Aesthetic — Pull focus (Structured) · 40-50 min

**CODE:** 🐂🌹🪡🔵

> "Follow the prescription. The numbers build the habit."

*Kettlebell Strength: Linear progression tracking — same weight, same reps, add load when form holds*
═══
## 1) ♨️ Warm-Up — 🦢 plico
Subcode: 🐂🌹🪡🔵 (Warm-Up | Pull | Aesthetic | Structured)
├─ 12 🪡 Single-Arm Lat Pulldown (prescribed, track sets and reps)
│  Set 1: 🐂 45% × 12 (pattern prep)
Rest: 60s
═══
## 2) 🔢 Fundamentals
Subcode: 🐂🌹🪡🔵 (Fundamentals | Pull | Aesthetic | Structured)
├─ 12 🪡 Single-Arm Dumbbell Row (slow, own each position)
│  Set 1: 🐂 45% × 12 (grounding)
│  Set 2: 🐂 50% × 12 (pattern lock)
Rest: 75s
═══
## 3) 🧈 Bread & Butter
Subcode: 🐂🌹🪡🔵 (Bread & Butter | Pull | Aesthetic | Structured)
├─ 11 🪡 Archer Pull-Up (prescribed, track sets and reps)
│  Set 1: 🐂 60% × 11 (build set)
│  Set 2: 🐂 65% × 11 (working set)
│  Set 3: 🐂 65% × 11 (repeat quality)
Rest: 75s
═══
## 4) 🧩 Supplemental
Subcode: 🐂🌹🪡🔵 (Supplemental | Pull | Aesthetic | Structured)
├─ 13 🪡 Plank Row (Single-Arm) (full range, different angle)
│  Set 1: 🐂 55% × 13 (support volume)
│  Set 2: 🐂 55% × 13 (non-redundant)
Rest: 60s
═══
## 5) 🧬 Imprint
Subcode: 🐂🌹🪡🔵 (Imprint | Pull | Aesthetic | Structured)
├─ 15 🪡 Standing Cable Row (Single-Arm) (high rep, low load, neural memory)
│  Set 1: 🐂 40% × 15 (lock the pattern)
Rest: 60s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🐂🌹🪡🔵 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Log the numbers. Consistency at sub-maximal load builds the strongest base.
