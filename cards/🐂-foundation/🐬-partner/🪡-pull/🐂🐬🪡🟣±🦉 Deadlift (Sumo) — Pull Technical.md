---
zip: 🐂🐬🪡🟣
operator: 🦉 logos
status: GENERATED
deck: 06
order: 🐂 Foundation | <=65% | 8-15 reps | 60-90s | CNS: Low
axis: 🐬 Partner | Social training context. Spottable, alternating, teachable.
type: 🪡 Pull | Lats, rear delts, biceps, traps, erectors
color: 🟣 Technical | Tier 2-5 | GOLD: Yes | Precision. Lower volume, extended rest, quality focus.
blocks: ♨️ 🔢/🛠 🧈 🧩 🧬 🚂
---
# 🪡 Deadlift (Sumo) — Pull Technical 🪡

## Foundation Partner — Pull focus (Technical) · 40-50 min

**CODE:** 🐂🐬🪡🟣

> "Learn the pattern at sub-maximal load. Own the positions before adding weight."
═══
## 1) ♨️ Warm-Up — 🦉 logos
Subcode: 🐂🐬🪡🟣 (Warm-Up | Pull | Partner | Technical)
├─ 12 🪡 Wide-Grip Pull-Up (Teres Focus) (precision, quality over volume)
│  Set 1: 🐂 45% × 12 (pattern prep)
Rest: 60s
═══
## 2) 🔢 Fundamentals
Subcode: 🐂🐬🪡🟣 (Fundamentals | Pull | Partner | Technical)
├─ 12 🪡 Chest-Supported Row (Elbow Out) (slow, own each position)
│  Set 1: 🐂 45% × 12 (grounding)
│  Set 2: 🐂 50% × 12 (pattern lock)
Rest: 75s
═══
## 3) 🧈 Bread & Butter
Subcode: 🐂🐬🪡🟣 (Bread & Butter | Pull | Partner | Technical)
├─ 11 🪡 Deadlift (Sumo) (precision, quality over volume)
│  Set 1: 🐂 60% × 11 (build set)
│  Set 2: 🐂 65% × 11 (working set)
│  Set 3: 🐂 65% × 11 (repeat quality)
Rest: 75s
═══
## 4) 🧩 Supplemental
Subcode: 🐂🐬🪡🟣 (Supplemental | Pull | Partner | Technical)
├─ 13 🪡 Cable Row (High Elbow) (full range, different angle)
│  Set 1: 🐂 55% × 13 (support volume)
│  Set 2: 🐂 55% × 13 (non-redundant)
Rest: 60s
═══
## 5) 🧬 Imprint
Subcode: 🐂🐬🪡🟣 (Imprint | Pull | Partner | Technical)
├─ 15 🪡 Scapular Push-Up (high rep, low load, neural memory)
│  Set 1: 🐂 40% × 15 (lock the pattern)
Rest: 60s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🐂🐬🪡🟣 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
The pattern owns the session. Add load only when positions are automatic.
