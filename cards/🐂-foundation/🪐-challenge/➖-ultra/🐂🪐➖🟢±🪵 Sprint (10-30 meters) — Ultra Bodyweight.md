---
zip: 🐂🪐➖🟢
operator: 🪵 teneo
status: GENERATED
deck: 04
order: 🐂 Foundation | <=65% | 8-15 reps | 60-90s | CNS: Low
axis: 🪐 Challenge | Hardest variation at any level. Deficit, pause, tempo, bands.
type: ➖ Ultra | Cardiovascular system
color: 🟢 Bodyweight | Tier 0-2 | GOLD: No | No gym required. Park, hotel, living room.
blocks: ♨️ 🔢/🛠 🧈 🧩 🧬 🚂
---
# ➖ Rowing Machine (Steady State) — Ultra Bodyweight ➖

## Foundation Challenge — Ultra focus (Bodyweight) · 40-50 min

**CODE:** 🐂🪐➖🟢

> "Learn the pattern with your own body first. External load comes later."

*Minimalist Hypertrophy: Bodyweight hypertrophy — high reps, slow tempo, squeeze and hold patterns*
═══
## 1) ♨️ Warm-Up — 🪵 teneo
Subcode: 🐂🪐➖🟢 (Warm-Up | Ultra | Challenge | Bodyweight)
├─ 12 ➖ Rowing Intervals (500m) (bodyweight, no external load needed)
│  Set 1: 🐂 45% × 12 (pattern prep)
Rest: 60s
═══
## 2) 🔢 Fundamentals
Subcode: 🐂🪐➖🟢 (Fundamentals | Ultra | Challenge | Bodyweight)
├─ 12 ➖ Rowing Intervals (1000m) (slow, own each position)
│  Set 1: 🐂 45% × 12 (grounding)
│  Set 2: 🐂 50% × 12 (pattern lock)
Rest: 75s
═══
## 3) 🧈 Bread & Butter
Subcode: 🐂🪐➖🟢 (Bread & Butter | Ultra | Challenge | Bodyweight)
├─ 11 ➖ Rowing Machine (Steady State) (bodyweight, no external load needed)
│  Set 1: 🐂 60% × 11 (build set)
│  Set 2: 🐂 65% × 11 (working set)
│  Set 3: 🐂 65% × 11 (repeat quality)
Rest: 75s
═══
## 4) 🧩 Supplemental
Subcode: 🐂🪐➖🟢 (Supplemental | Ultra | Challenge | Bodyweight)
├─ 13 ➖ Rowing Intervals (2000m) (full range, different angle)
│  Set 1: 🐂 55% × 13 (support volume)
│  Set 2: 🐂 55% × 13 (non-redundant)
Rest: 60s
═══
## 5) 🧬 Imprint
Subcode: 🐂🪐➖🟢 (Imprint | Ultra | Challenge | Bodyweight)
├─ 15 ➖ Rowing Sprints (250m) (high rep, low load, neural memory)
│  Set 1: 🐂 40% × 15 (lock the pattern)
Rest: 60s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🐂🪐➖🟢 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
If the pattern held at bodyweight, it will hold under load. Graduate when ready.
