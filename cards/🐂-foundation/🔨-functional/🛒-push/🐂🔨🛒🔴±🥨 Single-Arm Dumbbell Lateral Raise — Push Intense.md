---
zip: 🐂🔨🛒🔴
operator: 🥨 tendo
status: GENERATED
deck: 02
order: 🐂 Foundation | <=65% | 8-15 reps | 60-90s | CNS: Low
axis: 🔨 Functional | Unilateral, standing, athletic-transfer movements.
type: 🛒 Push | Chest, front delts, triceps
color: 🔴 Intense | Tier 2-4 | GOLD: Yes | Maximum effort. High volume. Reduced rest. Supersets OK.
blocks: ♨️ 🔢/🛠 🧈 🧩 🧬 🚂
---
# 🛒 Single-Arm Dumbbell Lateral Raise — Push Intense 🛒

## Foundation Functional — Push focus (Intense) · 40-50 min

**CODE:** 🐂🔨🛒🔴

> "Sub-maximal load, high effort. Earn every rep with form."
═══
## 1) ♨️ Warm-Up — 🥨 tendo
Subcode: 🐂🔨🛒🔴 (Warm-Up | Push | Functional | Intense)
├─ 12 🛒 Single-Arm Cable Chest Fly (high effort, push the pace)
│  Set 1: 🐂 45% × 12 (pattern prep)
Rest: 30s
═══
## 2) 🔢 Fundamentals
Subcode: 🐂🔨🛒🔴 (Fundamentals | Push | Functional | Intense)
├─ 12 🛒 Alternating Dumbbell Front Raise (slow, own each position)
│  Set 1: 🐂 45% × 12 (grounding)
│  Set 2: 🐂 50% × 12 (pattern lock)
Rest: 37s
═══
## 3) 🧈 Bread & Butter
Subcode: 🐂🔨🛒🔴 (Bread & Butter | Push | Functional | Intense)
├─ 11 🛒 Single-Arm Dumbbell Lateral Raise (high effort, push the pace)
│  Set 1: 🐂 60% × 11 (build set)
│  Set 2: 🐂 65% × 11 (working set)
│  Set 3: 🐂 65% × 11 (repeat quality)
Rest: 37s
═══
## 4) 🧩 Supplemental
Subcode: 🐂🔨🛒🔴 (Supplemental | Push | Functional | Intense)
├─ 13 🛒 Alternating Dumbbell Curl (full range, different angle)
│  Set 1: 🐂 55% × 13 (support volume)
│  Set 2: 🐂 55% × 13 (non-redundant)
Rest: 30s
═══
## 5) 🧬 Imprint
Subcode: 🐂🔨🛒🔴 (Imprint | Push | Functional | Intense)
├─ 15 🛒 Medicine Ball Chest Pass (high rep, low load, neural memory)
│  Set 1: 🐂 40% × 15 (lock the pattern)
Rest: 30s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🐂🔨🛒🔴 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Effort at low load reveals form breaks. Fix them before adding weight.
