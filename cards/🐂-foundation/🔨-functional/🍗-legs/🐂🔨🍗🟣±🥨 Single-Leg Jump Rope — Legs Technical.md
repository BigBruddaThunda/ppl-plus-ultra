---
zip: 🐂🔨🍗🟣
operator: 🥨 tendo
status: GENERATED
deck: 02
order: 🐂 Foundation | <=65% | 8-15 reps | 60-90s | CNS: Low
axis: 🔨 Functional | Unilateral, standing, athletic-transfer movements.
type: 🍗 Legs | Quads, hamstrings, glutes, calves
color: 🟣 Technical | Tier 2-5 | GOLD: Yes | Precision. Lower volume, extended rest, quality focus.
blocks: ♨️ 🔢/🛠 🧈 🧩 🧬 🚂
---
# 🍗 Single-Leg Jump Rope — Legs Technical 🍗

## Foundation Functional — Legs focus (Technical) · 40-50 min

**CODE:** 🐂🔨🍗🟣

> "Quality of position matters more than speed of completion."
═══
## 1) ♨️ Warm-Up — 🥨 tendo
Subcode: 🐂🔨🍗🟣 (Warm-Up | Legs | Functional | Technical)
├─ 12 🍗 Single-Leg Pogo Hops (precision, quality over volume)
│  Set 1: 🐂 45% × 12 (pattern prep)
Rest: 90s
═══
## 2) ▶️ Primer
Subcode: 🐂🔨🍗🟣 (Primer | Legs | Functional | Technical)
├─ 11 🍗 Deep Squat Hold (Ankle Mobility) (tight setup, crisp intent)
│  Set 1: 🐂 55% × 11 (activation)
Rest: 112s
═══
## 3) 🧈 Bread & Butter
Subcode: 🐂🔨🍗🟣 (Bread & Butter | Legs | Functional | Technical)
├─ 11 🍗 Single-Leg Jump Rope (precision, quality over volume)
│  Set 1: 🐂 60% × 11 (build set)
│  Set 2: 🐂 65% × 11 (working set)
│  Set 3: 🐂 65% × 11 (repeat quality)
Rest: 112s
═══
## 4) 🧩 Supplemental
Subcode: 🐂🔨🍗🟣 (Supplemental | Legs | Functional | Technical)
├─ 13 🍗 Single-Leg Extension (full range, different angle)
│  Set 1: 🐂 55% × 13 (support volume)
│  Set 2: 🐂 55% × 13 (non-redundant)
Rest: 90s
═══
## 5) 🪫 Release
Subcode: 🐂🔨🍗🟣 (Release | Legs | Functional | Technical)
├─ 12 🍗 Single-Leg Good Morning (smooth tempo, downshift)
│  Set 1: 🐂 45% × 12 (deload)
Rest: 90s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🐂🔨🍗🟣 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Note which positions felt uncertain. Those are your next session's priority.
