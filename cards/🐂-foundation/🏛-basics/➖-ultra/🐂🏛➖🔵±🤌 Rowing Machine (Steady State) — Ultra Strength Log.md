---
zip: 🐂🏛➖🔵
operator: 🤌 facio
status: GENERATED
deck: 01
updated: 2026-03-06
order: 🐂 Foundation | <=65% | 8-15 reps | 60-90s | CNS: Low
axis: 🏛 Basics | Bilateral, stable, barbell-first classics. Compound over isolation.
type: ➖ Ultra | Cardiovascular system and conditioning engine
color: 🔵 Strength Log | Tier 0-3 | GOLD: No
blocks: ♨️ → 🔢/🛠 → 🧈 → 🧩 → 🧬 → 🚂 → 🧮
---
# ➖ Rowing Machine (Steady State) — Ultra Structured ➖

## Foundation Basics — Ultra focus (Structured) · 40-50 min

**CODE:** 🐂🏛➖🔵

> "Follow the prescription. The numbers build the habit."

*General Strength: Linear progression tracking — same weight, same reps, add load when form holds*
═══
## 1) ♨️ Warm-Up — 🤌 facio
Subcode: 🐂🏛➖🔵 (Warm-Up | Ultra | Basics | Structured)
├─ 12 ➖ Rowing Intervals (500m) (prescribed, track sets and reps)
│  Set 1: 🐂 45% × 12 (pattern prep)
Rest: 60s
═══
## 2) 🔢 Fundamentals
Subcode: 🐂🏛➖🔵 (Fundamentals | Ultra | Basics | Structured)
├─ 12 ➖ Rowing Intervals (1000m) (slow, own each position)
│  Set 1: 🐂 45% × 12 (grounding)
│  Set 2: 🐂 50% × 12 (pattern lock)
Rest: 75s
═══
## 3) 🧈 Bread & Butter
Subcode: 🐂🏛➖🔵 (Bread & Butter | Ultra | Basics | Structured)
├─ 11 ➖ Rowing Machine (Steady State) (prescribed, track sets and reps)
│  Set 1: 🐂 60% × 11 (build set)
│  Set 2: 🐂 65% × 11 (working set)
│  Set 3: 🐂 65% × 11 (repeat quality)
Rest: 75s
═══
## 4) 🧩 Supplemental
Subcode: 🐂🏛➖🔵 (Supplemental | Ultra | Basics | Structured)
├─ 13 ➖ Rowing Intervals (2000m) (full range, different angle)
│  Set 1: 🐂 55% × 13 (support volume)
│  Set 2: 🐂 55% × 13 (non-redundant)
Rest: 60s
═══
## 5) 🧬 Imprint
Subcode: 🐂🏛➖🔵 (Imprint | Ultra | Basics | Structured)
├─ 15 ➖ Rowing Sprints (250m) (high rep, low load, neural memory)
│  Set 1: 🐂 40% × 15 (lock the pattern)
Rest: 60s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🐂🏛➖🔵 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Log the numbers. Consistency at sub-maximal load builds the strongest base.
