---
zip: 🐂🏛➕⚫
operator: 📍 pono
status: GENERATED
deck: 01
updated: 2026-03-06
order: 🐂 Foundation | <=65% | 8-15 reps | 60-90s | CNS: Low
axis: 🏛 Basics | Bilateral, stable, barbell-first classics. Compound over isolation.
type: ➕ Plus | Full-body power and core control
color: ⚫ Coaching Session | Tier 0-3 | GOLD: No
blocks: ♨️ → 🔢/🛠 → 🧈 → 🧩 → 🧬 → 🚂 → 🧮
---
# ➕ Hip Flexor Stretch (Half-Kneeling) — Plus Teaching ➕

## Foundation Basics — Plus focus (Teaching) · 40-50 min

**CODE:** 🐂🏛➕⚫

> "Practice the positions slowly. This is a coaching session, not a workout."

*Senior Strength: Teaches barbell setup, loading sequence, and safety positions for heavy compound lifts*
═══
## 1) ♨️ Warm-Up — 📍 pono
Subcode: 🐂🏛➕⚫ (Warm-Up | Plus | Basics | Teaching)
├─ 12 ➕ Aquaman (Alternating Superman) (coached, check form before adding load)
│  Set 1: 🐂 45% × 12 (pattern prep)
Rest: 90s
═══
## 2) 🛠 Craft
Subcode: 🐂🏛➕⚫ (Craft | Plus | Basics | Teaching)
├─ 11 ➕ Half-Kneeling Band Single-Arm Pulldown (coach: check grip width, elbow angle, foot pressure)
│  Set 1: 🐂 45% × 11 (practice the pattern, not the load)
│  Set 2: 🐂 50% × 11 (same cues, slightly heavier)
Rest: 112s
═══
## 3) 🧈 Bread & Butter
Subcode: 🐂🏛➕⚫ (Bread & Butter | Plus | Basics | Teaching)
├─ 11 ➕ Hip Flexor Stretch (Half-Kneeling) (coached, check form before adding load)
│  Set 1: 🐂 60% × 11 (build set)
│  Set 2: 🐂 65% × 11 (working set)
│  Set 3: 🐂 65% × 11 (repeat quality)
Rest: 112s
═══
## 4) 🧩 Supplemental
Subcode: 🐂🏛➕⚫ (Supplemental | Plus | Basics | Teaching)
├─ 13 ➕ Single-Leg Plank (full range, different angle)
│  Set 1: 🐂 55% × 13 (support volume)
│  Set 2: 🐂 55% × 13 (non-redundant)
Rest: 90s
═══
## 5) 🧬 Imprint
Subcode: 🐂🏛➕⚫ (Imprint | Plus | Basics | Teaching)
├─ 15 ➕ Single-Arm Single-Leg Plank (high rep, low load, neural memory)
│  Set 1: 🐂 40% × 15 (lock the pattern)
Rest: 90s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🐂🏛➕⚫ — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
The positions own the session. Add complexity only when coaching cues become automatic.
