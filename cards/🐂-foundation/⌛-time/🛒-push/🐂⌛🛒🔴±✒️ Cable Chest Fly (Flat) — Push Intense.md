---
zip: 🐂⌛🛒🔴
operator: ✒️ grapho
status: GENERATED
deck: 05
order: 🐂 Foundation | <=65% | 8-15 reps | 60-90s | CNS: Low
axis: ⌛ Time | Clock as training variable. EMOM, AMRAP, density, timed sets.
type: 🛒 Push | Chest, front delts, triceps
color: 🔴 Intense | Tier 2-4 | GOLD: Yes | Maximum effort. High volume. Reduced rest. Supersets OK.
blocks: ♨️ 🔢/🛠 🧈 🧩 🧬 🚂
---
# 🛒 Cable Chest Fly (Flat) — Push Intense 🛒

## Foundation Time — Push focus (Intense) · 40-50 min

**CODE:** 🐂⌛🛒🔴

> "Sub-maximal load, high effort. Earn every rep with form."
═══
## 1) ♨️ Warm-Up — ✒️ grapho
Subcode: 🐂⌛🛒🔴 (Warm-Up | Push | Time | Intense)
├─ 12 🛒 Medicine Ball Chest Pass (high effort, push the pace)
│  Set 1: 🐂 45% × 12 (pattern prep)
Rest: 30s
═══
## 2) 🔢 Fundamentals
Subcode: 🐂⌛🛒🔴 (Fundamentals | Push | Time | Intense)
├─ 12 🛒 Dumbbell Chest Fly (slow, own each position)
│  Set 1: 🐂 45% × 12 (grounding)
│  Set 2: 🐂 50% × 12 (pattern lock)
Rest: 37s
═══
## 3) 🧈 Bread & Butter
Subcode: 🐂⌛🛒🔴 (Bread & Butter | Push | Time | Intense)
├─ 11 🛒 Cable Chest Fly (Flat) (high effort, push the pace)
│  Set 1: 🐂 60% × 11 (build set)
│  Set 2: 🐂 65% × 11 (working set)
│  Set 3: 🐂 65% × 11 (repeat quality)
Rest: 37s
═══
## 4) 🧩 Supplemental
Subcode: 🐂⌛🛒🔴 (Supplemental | Push | Time | Intense)
├─ 13 🛒 Low-to-High Cable Fly (full range, different angle)
│  Set 1: 🐂 55% × 13 (support volume)
│  Set 2: 🐂 55% × 13 (non-redundant)
Rest: 30s
═══
## 5) 🧬 Imprint
Subcode: 🐂⌛🛒🔴 (Imprint | Push | Time | Intense)
├─ 15 🛒 Dumbbell Front Raise (high rep, low load, neural memory)
│  Set 1: 🐂 40% × 15 (lock the pattern)
Rest: 30s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🐂⌛🛒🔴 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Effort at low load reveals form breaks. Fix them before adding weight.
