---
zip: 🐂⌛➕🔴
operator: ✒️ grapho
status: GENERATED
deck: 05
order: 🐂 Foundation | <=65% | 8-15 reps | 60-90s | CNS: Low
axis: ⌛ Time | Clock as training variable. EMOM, AMRAP, density, timed sets.
type: ➕ Plus | Full body power, core
color: 🔴 Intense | Tier 2-4 | GOLD: Yes | Maximum effort. High volume. Reduced rest. Supersets OK.
blocks: ♨️ 🔢/🛠 🧈 🧩 🧬 🚂
---
# ➕ Neck Rotation with Band — Plus Intense ➕

## Foundation Time — Plus focus (Intense) · 40-50 min

**CODE:** 🐂⌛➕🔴

> "Sub-maximal load, high effort. Earn every rep with form."
═══
## 1) ♨️ Warm-Up — ✒️ grapho
Subcode: 🐂⌛➕🔴 (Warm-Up | Plus | Time | Intense)
├─ 12 ➕ Yoke Walk (high effort, push the pace)
│  Set 1: 🐂 45% × 12 (pattern prep)
Rest: 30s
═══
## 2) 🔢 Fundamentals
Subcode: 🐂⌛➕🔴 (Fundamentals | Plus | Time | Intense)
├─ 12 ➕ Neck Rotation (Bodyweight) (slow, own each position)
│  Set 1: 🐂 45% × 12 (grounding)
│  Set 2: 🐂 50% × 12 (pattern lock)
Rest: 37s
═══
## 3) 🧈 Bread & Butter
Subcode: 🐂⌛➕🔴 (Bread & Butter | Plus | Time | Intense)
├─ 11 ➕ Neck Rotation with Band (high effort, push the pace)
│  Set 1: 🐂 60% × 11 (build set)
│  Set 2: 🐂 65% × 11 (working set)
│  Set 3: 🐂 65% × 11 (repeat quality)
Rest: 37s
═══
## 4) 🧩 Supplemental
Subcode: 🐂⌛➕🔴 (Supplemental | Plus | Time | Intense)
├─ 13 ➕ Wide-Grip Lat Pulldown (Teres Focus) (full range, different angle)
│  Set 1: 🐂 55% × 13 (support volume)
│  Set 2: 🐂 55% × 13 (non-redundant)
Rest: 30s
═══
## 5) 🧬 Imprint
Subcode: 🐂⌛➕🔴 (Imprint | Plus | Time | Intense)
├─ 15 ➕ Good Morning (Barbell) (high rep, low load, neural memory)
│  Set 1: 🐂 40% × 15 (lock the pattern)
Rest: 30s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🐂⌛➕🔴 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Effort at low load reveals form breaks. Fix them before adding weight.
