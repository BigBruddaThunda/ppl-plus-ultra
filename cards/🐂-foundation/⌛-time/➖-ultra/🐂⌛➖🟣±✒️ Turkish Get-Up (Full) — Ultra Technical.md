---
zip: 🐂⌛➖🟣
operator: ✒️ grapho
status: GENERATED
deck: 05
order: 🐂 Foundation | <=65% | 8-15 reps | 60-90s | CNS: Low
axis: ⌛ Time | Clock as training variable. EMOM, AMRAP, density, timed sets.
type: ➖ Ultra | Cardiovascular system
color: 🟣 Technical | Tier 2-5 | GOLD: Yes | Precision. Lower volume, extended rest, quality focus.
blocks: ♨️ 🔢/🛠 🧈 🧩 🧬 🚂
---
# ➖ Turkish Get-Up (Full) — Ultra Technical ➖

## Foundation Time — Ultra focus (Technical) · 40-50 min

**CODE:** 🐂⌛➖🟣

> "Learn the pattern at sub-maximal load. Own the positions before adding weight."
═══
## 1) ♨️ Warm-Up — ✒️ grapho
Subcode: 🐂⌛➖🟣 (Warm-Up | Ultra | Time | Technical)
├─ 12 ➖ Turkish Get-Up (Full) (precision, quality over volume)
│  Set 1: 🐂 45% × 12 (pattern prep)
Rest: 60s
═══
## 2) 🔢 Fundamentals
Subcode: 🐂⌛➖🟣 (Fundamentals | Ultra | Time | Technical)
├─ 12 ➖ Kettlebell Goblet Squat (slow, own each position)
│  Set 1: 🐂 45% × 12 (grounding)
│  Set 2: 🐂 50% × 12 (pattern lock)
Rest: 75s
═══
## 3) 🧈 Bread & Butter
Subcode: 🐂⌛➖🟣 (Bread & Butter | Ultra | Time | Technical)
├─ 11 ➖ Turkish Get-Up (Full) (precision, quality over volume)
│  Set 1: 🐂 60% × 11 (build set)
│  Set 2: 🐂 65% × 11 (working set)
│  Set 3: 🐂 65% × 11 (repeat quality)
Rest: 75s
═══
## 4) 🧩 Supplemental
Subcode: 🐂⌛➖🟣 (Supplemental | Ultra | Time | Technical)
├─ 13 ➖ Half Turkish Get-Up (To Elbow) (full range, different angle)
│  Set 1: 🐂 55% × 13 (support volume)
│  Set 2: 🐂 55% × 13 (non-redundant)
Rest: 60s
═══
## 5) 🧬 Imprint
Subcode: 🐂⌛➖🟣 (Imprint | Ultra | Time | Technical)
├─ 15 ➖ Turkish Get-Up (To Kneeling) (high rep, low load, neural memory)
│  Set 1: 🐂 40% × 15 (lock the pattern)
Rest: 60s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🐂⌛➖🟣 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
The pattern owns the session. Add load only when positions are automatic.
