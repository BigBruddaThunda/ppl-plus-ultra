---
zip: 🖼🔨🛒⚪
operator: 🧸 fero
status: GENERATED
deck: 38
order: 🖼 Restoration | <=55% | 12-15 reps | 60s | CNS: Low
axis: 🔨 Functional | Unilateral, standing, athletic-transfer movements.
type: 🛒 Push | Chest, front delts, triceps
color: ⚪ Mindful | Tier 0-3 | GOLD: No | Slow tempo (4s eccentrics). Extended rest (2+ min). Breathing cues.
blocks: 🎯 🪫 🧈 🧬 🚂
---
# 🛒 Plyometric Push-Up (Hands Leave Ground) — Push Mindful 🛒

## Restoration Functional — Push focus (Mindful) · 30-40 min

**CODE:** 🖼🔨🛒⚪

> "Breathe into each position. Leave fresher than you entered."

*Fasting Cardio + Strength: Zone 2 steady state — conversational pace, extended duration, aerobic base*
═══
## 1) 🎯 Intention — 🧸 fero

> "Breathe into each position. Leave fresher than you entered."
═══
## 2) 🪫 Release
Subcode: 🖼🔨🛒⚪ (Release | Push | Functional | Mindful)
├─ 12 🛒 American Swing (Overhead) (parasympathetic, tension down, 4s eccentric)
│  Set 1: 🖼 40% × 12 (deload)
Rest: 120s
═══
## 3) 🧈 Bread & Butter
Subcode: 🖼🔨🛒⚪ (Bread & Butter | Push | Functional | Mindful)
├─ 13 🛒 Plyometric Push-Up (Hands Leave Ground) (4s eccentric, breath-paced)
│  Set 1: 🖼 55% × 13 (slow, feel each rep)
│  Set 2: 🖼 55% × 13 (same tempo, same breath)
Rest: 120s
═══
## 4) 🧬 Imprint
Subcode: 🖼🔨🛒⚪ (Imprint | Push | Functional | Mindful)
├─ 15 🛒 Heavy Swing (Power Focus) (4s eccentric, breath-paced)
│  Set 1: 🖼 35% × 15 (lock the pattern)
Rest: 120s
═══
## 5) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🖼🔨🛒⚪ — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Notice what released. Carry that awareness into the next 24 hours.
