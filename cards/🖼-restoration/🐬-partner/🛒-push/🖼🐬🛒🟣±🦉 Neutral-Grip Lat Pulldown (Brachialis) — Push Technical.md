---
zip: 🖼🐬🛒🟣
operator: 🦉 logos
status: GENERATED
deck: 42
order: 🖼 Restoration | <=55% | 12-15 reps | 60s | CNS: Low
axis: 🐬 Partner | Social training context. Spottable, alternating, teachable.
type: 🛒 Push | Chest, front delts, triceps
color: 🟣 Technical | Tier 2-5 | GOLD: Yes | Precision. Lower volume, extended rest, quality focus.
blocks: 🎯 🪫 🧈 🧬 🚂
---
# 🛒 Neutral-Grip Lat Pulldown (Brachialis) — Push Technical 🛒

## Restoration Partner — Push focus (Technical) · 30-40 min

**CODE:** 🖼🐬🛒🟣

> "Leave fresher than you entered. Recovery is the work."
═══
## 1) 🎯 Intention

> "Leave fresher than you entered. Recovery is the work."
═══
## 2) 🪫 Release
Subcode: 🖼🐬🛒🟣 (Release | Push | Partner | Technical)
├─ 12 🛒 Preacher Curl Machine (parasympathetic, tension down)
│  Set 1: 🖼 40% × 12 (deload)
Rest: 60s
═══
## 3) 🧈 Bread & Butter
Subcode: 🖼🐬🛒🟣 (Bread & Butter | Push | Partner | Technical)
├─ 13 🛒 Neutral-Grip Lat Pulldown (Brachialis) (precision, quality over volume)
│  Set 1: 🖼 55% × 13 (slow, feel each rep)
│  Set 2: 🖼 55% × 13 (same tempo, same breath)
Rest: 60s
═══
## 4) 🧬 Imprint
Subcode: 🖼🐬🛒🟣 (Imprint | Push | Partner | Technical)
├─ 17 🛒 Alternating Dumbbell Front Raise (high rep, low load, neural memory)
│  Set 1: 🖼 35% × 17 (lock the pattern)
Rest: 60s
═══
## 5) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🖼🐬🛒🟣 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Notice what released. Carry that awareness into the next 24 hours.
