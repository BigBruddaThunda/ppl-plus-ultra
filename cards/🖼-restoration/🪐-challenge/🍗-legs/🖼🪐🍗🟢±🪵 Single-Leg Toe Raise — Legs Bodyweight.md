---
zip: 🖼🪐🍗🟢
operator: 🪵 teneo
status: GENERATED
deck: 40
order: 🖼 Restoration | <=55% | 12-15 reps | 60s | CNS: Low
axis: 🪐 Challenge | Hardest variation at any level. Deficit, pause, tempo, bands.
type: 🍗 Legs | Quads, hamstrings, glutes, calves
color: 🟢 Bodyweight | Tier 0-2 | GOLD: No | No gym required. Park, hotel, living room.
blocks: 🎯 🪫 🧈 🧬 🚂
---
# 🍗 Single-Leg Jump Rope — Legs Bodyweight 🍗

## Restoration Challenge — Legs focus (Bodyweight) · 30-40 min

**CODE:** 🖼🪐🍗🟢

> "Restore with what your body provides. Floor work, breath, gentle movement."

*Minimalist Hypertrophy: Bodyweight hypertrophy — high reps, slow tempo, squeeze and hold patterns*
═══
## 1) 🎯 Intention

> "Restore with what your body provides. Floor work, breath, gentle movement."
═══
## 2) 🪫 Release
Subcode: 🖼🪐🍗🟢 (Release | Legs | Challenge | Bodyweight)
├─ 12 🍗 Goblet Squat (Deep Glute Stretch) (parasympathetic, tension down, 4s eccentric)
│  Set 1: 🖼 40% × 12 (deload)
Rest: 60s
═══
## 3) 🧈 Bread & Butter
Subcode: 🖼🪐🍗🟢 (Bread & Butter | Legs | Challenge | Bodyweight)
├─ 13 🍗 Single-Leg Jump Rope (bodyweight, no external load needed)
│  Set 1: 🖼 55% × 13 (slow, feel each rep)
│  Set 2: 🖼 55% × 13 (same tempo, same breath)
Rest: 60s
═══
## 4) 🧬 Imprint
Subcode: 🖼🪐🍗🟢 (Imprint | Legs | Challenge | Bodyweight)
├─ 15 🍗 Conventional Deadlift (Glute Load) (high rep, low load, neural memory)
│  Set 1: 🖼 35% × 15 (lock the pattern)
Rest: 60s
═══
## 5) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🖼🪐🍗🟢 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Floor work and breath are always available. Use them daily.
