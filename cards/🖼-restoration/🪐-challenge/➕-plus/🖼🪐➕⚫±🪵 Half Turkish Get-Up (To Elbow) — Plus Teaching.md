---
zip: 🖼🪐➕⚫
operator: 🪵 teneo
status: GENERATED
deck: 40
order: 🖼 Restoration | <=55% | 12-15 reps | 60s | CNS: Low
axis: 🪐 Challenge | Hardest variation at any level. Deficit, pause, tempo, bands.
type: ➕ Plus | Full body power, core
color: ⚫ Teaching | Tier 2-3 | GOLD: No | Extra rest, coaching cues, comprehension over exertion.
blocks: 🎯 🪫 🧈 🧬 🚂
---
# ➕ Pull-Up (Overhand Grip) — Plus Teaching ➕

## Restoration Challenge — Plus focus (Teaching) · 30-40 min

**CODE:** 🖼🪐➕⚫

> "Learn what recovery feels like. This is a teaching session for your nervous system."

*Olympic Lifting: Teaches the craft-specific positions, timing, and coordination demands*
═══
## 1) 🎯 Intention

> "Learn what recovery feels like. This is a teaching session for your nervous system."
═══
## 2) 🛠 Craft
Subcode: 🖼🪐➕⚫ (Craft | Plus | Challenge | Teaching)
├─ 13 ➕ Wide-Grip Pull-Up (coach: check grip width, elbow angle, foot pressure)
│  Set 1: 🖼 40% × 13 (practice the pattern, not the load)
│  Set 2: 🖼 45% × 13 (same cues, slightly heavier)
Rest: 90s
═══
## 3) 🧈 Bread & Butter
Subcode: 🖼🪐➕⚫ (Bread & Butter | Plus | Challenge | Teaching)
├─ 13 ➕ Pull-Up (Overhand Grip) (coached, check form before adding load)
│  Set 1: 🖼 55% × 13 (slow, feel each rep)
│  Set 2: 🖼 55% × 13 (same tempo, same breath)
Rest: 90s
═══
## 4) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🖼🪐➕⚫ — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Learning to recover is a skill. Note which positions brought the most relief.
