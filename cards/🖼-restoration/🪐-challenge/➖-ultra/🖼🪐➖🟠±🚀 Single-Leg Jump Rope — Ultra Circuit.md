---
zip: 🖼🪐➖🟠
operator: 🚀 mitto
status: GENERATED
deck: 40
order: 🖼 Restoration | <=55% | 12-15 reps | 60s | CNS: Low
axis: 🪐 Challenge | Hardest variation at any level. Deficit, pause, tempo, bands.
type: ➖ Ultra | Cardiovascular system
color: 🟠 Circuit | Tier 0-3 | GOLD: No | Station-based timed rotation. No barbells. Loop logic required.
blocks: 🎯 🪫 🧈 🧬 🚂
---
# ➖ Single-Leg Jump Rope — Ultra Circuit ➖

## Restoration Challenge — Ultra focus (Circuit) · 30-40 min

**CODE:** 🖼🪐➖🟠

> "Leave fresher than you entered. Recovery is the work."
═══
## 1) 🎯 Intention

> "Leave fresher than you entered. Recovery is the work."
═══
## 2) 🪫 Release
Subcode: 🖼🪐➖🟠 (Release | Ultra | Challenge | Circuit)
├─ 12 ➖ Sprint (30-60 meters) (parasympathetic, tension down)
│  Set 1: 🖼 40% × 12 (deload)
Rest: 60s
═══
## 3) 🎱 ARAM — Circuit Loop
Subcode: 🖼🪐➖🟠 (ARAM | Ultra | Challenge | Circuit)
┌─ 3 rounds, rotate through stations. No two adjacent stations same muscle group.
│  Station 1: ➖ Single-Leg Jump Rope × 13 (station rotation, keep moving)
│  Station 2: ➖ Single-Leg Jump Rope × 13 (station rotation, keep moving)
│  Station 3: ➖ Kettlebell Goblet Squat × 13 (station rotation, keep moving)
│  Station 4: ➖ Sprint (10-30 meters) × 13 (station rotation, keep moving)
│  Transition: 15s between stations
│  Round rest: 60s
Rest: 90s after final round
═══
## 4) 🧬 Imprint
Subcode: 🖼🪐➖🟠 (Imprint | Ultra | Challenge | Circuit)
├─ 17 ➖ Single-Leg Jump Rope (high rep, low load, neural memory)
│  Set 1: 🖼 35% × 17 (lock the pattern)
Rest: 60s
═══
## 5) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🖼🪐➖🟠 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Notice what released. Carry that awareness into the next 24 hours.
