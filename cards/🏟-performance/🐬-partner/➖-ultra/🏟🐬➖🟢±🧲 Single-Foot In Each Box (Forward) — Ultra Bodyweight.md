---
zip: 🏟🐬➖🟢
operator: 🧲 capio
status: GENERATED
deck: 24
order: 🏟 Performance | 85-100%+ | 1-3 reps | Full recovery | CNS: High
axis: 🐬 Partner | Social training context. Spottable, alternating, teachable.
type: ➖ Ultra | Cardiovascular system
color: 🟢 Bodyweight | Tier 0-2 | GOLD: No | No gym required. Park, hotel, living room.
blocks: ♨️ 🪜 🧈 🚂
---
# ➖ Single-Leg Hops Through Ladder — Ultra Bodyweight ➖

## Performance Partner — Ultra focus (Bodyweight) · 25-35 min

**CODE:** 🏟🐬➖🟢

> "Test bodyweight capacity. How many, how fast, how clean."

*General Functional Fitness: Bodyweight functional — park, field, living room — prove the pattern works anywhere*
═══
## 1) ♨️ Warm-Up — 🧲 capio
Subcode: 🏟🐬➖🟢 (Warm-Up | Ultra | Partner | Bodyweight)
├─ 3 ➖ Partner Toss (Alternating) (bodyweight, no external load needed)
│  Set 1: 🏟 72% × 3 (pattern prep)
Rest: 90s
═══
## 2) 🪜 Progression
Subcode: 🏟🐬➖🟢 (Progression | Ultra | Partner | Bodyweight)
├─ 2 ➖ Single-Leg Hops Through Ladder (ramp to test weight)
│  Set 1: 🏟 77% × 3 (opener)
│  Set 2: 🏟 87% × 2 (bridge)
Rest: Full recovery
═══
## 3) 🧈 Bread & Butter
Subcode: 🏟🐬➖🟢 (Bread & Butter | Ultra | Partner | Bodyweight)
├─ 2 ➖ Single-Leg Hops Through Ladder (bodyweight, no external load needed)
│  Set 1: 🏟 92% × 2 (test attempt)
│  Set 2: 🏟 97% × 2 (max attempt)
Rest: Full recovery
═══
## 4) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🏟🐬➖🟢 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Record the bodyweight test number. That is the session.
