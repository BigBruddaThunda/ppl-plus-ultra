---
zip: 🏟🐬🛒🟢
operator: 🧲 capio
status: GENERATED
deck: 24
order: 🏟 Performance | 85-100%+ | 1-3 reps | Full recovery | CNS: High
axis: 🐬 Partner | Social training context. Spottable, alternating, teachable.
type: 🛒 Push | Chest, front delts, triceps
color: 🟢 Bodyweight | Tier 0-2 | GOLD: No | No gym required. Park, hotel, living room.
blocks: ♨️ 🪜 🧈 🚂
---
# 🛒 Half-Kneeling Cable Press — Push Bodyweight 🛒

## Performance Partner — Push focus (Bodyweight) · 25-35 min

**CODE:** 🏟🐬🛒🟢

> "Test bodyweight capacity. How many, how fast, how clean."
═══
## 1) ♨️ Warm-Up — 🧲 capio
Subcode: 🏟🐬🛒🟢 (Warm-Up | Push | Partner | Bodyweight)
├─ 3 🛒 Single-Arm Incline Dumbbell Press (bodyweight, no external load needed)
│  Set 1: 🏟 72% × 3 (pattern prep)
Rest: 90s
═══
## 2) 🪜 Progression
Subcode: 🏟🐬🛒🟢 (Progression | Push | Partner | Bodyweight)
├─ 2 🛒 Half-Kneeling Cable Press (ramp to test weight)
│  Set 1: 🏟 77% × 3 (opener)
│  Set 2: 🏟 87% × 2 (bridge)
Rest: Full recovery
═══
## 3) 🧈 Bread & Butter
Subcode: 🏟🐬🛒🟢 (Bread & Butter | Push | Partner | Bodyweight)
├─ 2 🛒 Half-Kneeling Cable Press (bodyweight, no external load needed)
│  Set 1: 🏟 92% × 2 (test attempt)
│  Set 2: 🏟 97% × 2 (max attempt)
Rest: Full recovery
═══
## 4) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🏟🐬🛒🟢 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Record the bodyweight test number. That is the session.
