---
zip: 🏟🔨🪡🟢
operator: 🧸 fero
status: GENERATED
deck: 20
order: 🏟 Performance | 85-100%+ | 1-3 reps | Full recovery | CNS: High
axis: 🔨 Functional | Unilateral, standing, athletic-transfer movements.
type: 🪡 Pull | Lats, rear delts, biceps, traps, erectors
color: 🟢 Bodyweight | Tier 0-2 | GOLD: No | No gym required. Park, hotel, living room.
blocks: ♨️ 🪜 🧈 🚂
---
# 🪡 One-Arm Deadlift — Pull Bodyweight 🪡

## Performance Functional — Pull focus (Bodyweight) · 25-35 min

**CODE:** 🏟🔨🪡🟢

> "Test. Record. Leave. No junk volume after the attempt."
═══
## 1) ♨️ Warm-Up — 🧸 fero
Subcode: 🏟🔨🪡🟢 (Warm-Up | Pull | Functional | Bodyweight)
├─ 3 🪡 Single-Arm Row (Rhomboid Emphasis) (bodyweight, no external load needed)
│  Set 1: 🏟 72% × 3 (pattern prep)
Rest: 90s
═══
## 2) 🪜 Progression
Subcode: 🏟🔨🪡🟢 (Progression | Pull | Functional | Bodyweight)
├─ 2 🪡 One-Arm Deadlift (ramp to test weight)
│  Set 1: 🏟 77% × 3 (opener)
│  Set 2: 🏟 87% × 2 (bridge)
Rest: Full recovery
═══
## 3) 🧈 Bread & Butter
Subcode: 🏟🔨🪡🟢 (Bread & Butter | Pull | Functional | Bodyweight)
├─ 2 🪡 One-Arm Deadlift (bodyweight, no external load needed)
│  Set 1: 🏟 92% × 2 (test attempt)
│  Set 2: 🏟 97% × 2 (max attempt)
Rest: Full recovery
═══
## 4) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🏟🔨🪡🟢 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Record the number. That is the session. Come back when recovered.
