---
zip: 🏟🌹🛒🔵
operator: 🦢 plico
status: GENERATED
deck: 21
order: 🏟 Performance | 85-100%+ | 1-3 reps | Full recovery | CNS: High
axis: 🌹 Aesthetic | Isolation, full ROM, mind-muscle connection. Feel over load.
type: 🛒 Push | Chest, front delts, triceps
color: 🔵 Structured | Tier 2-3 | GOLD: No | Prescribed sets/reps/rest. Trackable and repeatable.
blocks: ♨️ 🪜 🧈 🚂
---
# 🛒 Medicine Ball Slam (Chest Height) — Push Structured 🛒

## Performance Aesthetic — Push focus (Structured) · 25-35 min

**CODE:** 🏟🌹🛒🔵

> "Test. Record. Leave. No junk volume after the attempt."
═══
## 1) ♨️ Warm-Up — 🦢 plico
Subcode: 🏟🌹🛒🔵 (Warm-Up | Push | Aesthetic | Structured)
├─ 3 🛒 Medicine Ball Chest Pass (prescribed, track sets and reps)
│  Set 1: 🏟 72% × 3 (pattern prep)
Rest: 90s
═══
## 2) 🪜 Progression
Subcode: 🏟🌹🛒🔵 (Progression | Push | Aesthetic | Structured)
├─ 2 🛒 Medicine Ball Slam (Chest Height) (ramp to test weight)
│  Set 1: 🏟 77% × 3 (opener)
│  Set 2: 🏟 87% × 2 (bridge)
Rest: Full recovery
═══
## 3) 🧈 Bread & Butter
Subcode: 🏟🌹🛒🔵 (Bread & Butter | Push | Aesthetic | Structured)
├─ 2 🛒 Medicine Ball Slam (Chest Height) (prescribed, track sets and reps)
│  Set 1: 🏟 92% × 2 (test attempt)
│  Set 2: 🏟 97% × 2 (max attempt)
Rest: Full recovery
═══
## 4) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🏟🌹🛒🔵 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Record the number. That is the session. Come back when recovered.
