---
zip: ⛽🐬➕⚪
operator: 🧲 capio
status: GENERATED
deck: 12
order: ⛽ Strength | 75-85% | 4-6 reps | 3-4 min | CNS: High
axis: 🐬 Partner | Social training context. Spottable, alternating, teachable.
type: ➕ Plus | Partner-context strength emphasis
color: ⚪ Mindful | Partner deployment logic
blocks: ♨️ ▶️ 🧈 🧩 🪫 🚂
---
# ➕ Hip Flexor Stretch (Half-Kneeling) — Plus Mindful ➕

## Strength Partner — Plus focus (Mindful) · 50-65 min

**CODE:** ⛽🐬➕⚪

> "Four seconds down. Feel every degree of the range before you earn the drive."

*Injury Prevention / Prehab: Deep parasympathetic work — breathe into each position, leave fresher than you entered*
═══
## 1) ♨️ Warm-Up — 🧲 capio
Subcode: ⛽🐬➕⚪ (Warm-Up | Plus | Partner | Mindful)
├─ 6 ➕ Walking High Knees (4s eccentric, breath-paced)
│  Set 1: ⛽ 60% × 6 (pattern prep)
Rest: 180s
═══
## 2) 🧈 Bread & Butter
Subcode: ⛽🐬➕⚪ (Bread & Butter | Plus | Partner | Mindful)
├─ 5 ➕ Hip Flexor Stretch (Half-Kneeling) (4s eccentric, breath-paced)
│  Set 1: ⛽ 75% × 5 (build set)
│  Set 2: ⛽ 80% × 5 (working set)
│  Set 3: ⛽ 80% × 5 (repeat quality)
Rest: 360s
═══
## 3) 🧩 Supplemental
Subcode: ⛽🐬➕⚪ (Supplemental | Plus | Partner | Mindful)
├─ 6 ➕ Walking Leg Cradle (4s eccentric, breath-paced)
│  Set 1: ⛽ 70% × 6 (support volume)
│  Set 2: ⛽ 70% × 6 (non-redundant)
Rest: 240s
═══
## 4) 🪫 Release
Subcode: ⛽🐬➕⚪ (Release | Plus | Partner | Mindful)
├─ 6 ➕ Walking Quad Stretch (parasympathetic, tension down, 4s eccentric)
│  Set 1: ⛽ 60% × 6 (deload)
Rest: 240s
═══
## 5) 🧬 Imprint
Subcode: ⛽🐬➕⚪ (Imprint | Plus | Partner | Mindful)
├─ 6 ➕ Inchworm Walkout (4s eccentric, breath-paced)
│  Set 1: ⛽ 55% × 6 (lock the pattern)
Rest: 240s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → ⛽🐬➕⚪ — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Slow strength builds tendons, not just muscles. Trust the tempo.
