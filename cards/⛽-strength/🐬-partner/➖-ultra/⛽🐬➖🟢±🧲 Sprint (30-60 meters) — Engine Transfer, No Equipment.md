---
zip: ⛽🐬➖🟢
operator: 🧲 capio
status: GENERATED
deck: 12
order: ⛽ Strength | 75-85% | 4-6 reps | 3-4 min | CNS: High
axis: 🐬 Partner | Social training context. Spottable, alternating, teachable.
type: ➖ Ultra | Partner-context strength emphasis
color: 🟢 Bodyweight | Partner deployment logic
blocks: ♨️ ▶️ 🧈 🧩 🪫 🚂
---
# ➖ Single-Leg Hops Through Ladder — Ultra Bodyweight ➖

## Strength Partner — Ultra focus (Bodyweight) · 50-65 min

**CODE:** ⛽🐬➖🟢

> "Prove the pattern holds without external load. Bodyweight is the test."

*General Functional Fitness: Bodyweight functional — park, field, living room — prove the pattern works anywhere*
═══
## 1) ♨️ Warm-Up — 🧲 capio
Subcode: ⛽🐬➖🟢 (Warm-Up | Ultra | Partner | Bodyweight)
├─ 6 ➖ Partner Toss (Alternating) (bodyweight, no external load needed)
│  Set 1: ⛽ 60% × 6 (pattern prep)
Rest: 90s
═══
## 2) ▶️ Primer
Subcode: ⛽🐬➖🟢 (Primer | Ultra | Partner | Bodyweight)
├─ 5 ➖ Plant and Cut (Single-Leg Deceleration) (tight setup, crisp intent)
│  Set 1: ⛽ 70% × 5 (activation)
Rest: 120s
═══
## 3) 🧈 Bread & Butter
Subcode: ⛽🐬➖🟢 (Bread & Butter | Ultra | Partner | Bodyweight)
├─ 5 ➖ Single-Leg Hops Through Ladder (bodyweight, no external load needed)
│  Set 1: ⛽ 75% × 5 (build set)
│  Set 2: ⛽ 80% × 5 (working set)
│  Set 3: ⛽ 80% × 5 (repeat quality)
Rest: 180s
═══
## 4) 🧩 Supplemental
Subcode: ⛽🐬➖🟢 (Supplemental | Ultra | Partner | Bodyweight)
├─ 6 ➖ Layup Approach (Single-Leg Power) (full range, different angle)
│  Set 1: ⛽ 70% × 6 (support volume)
│  Set 2: ⛽ 70% × 6 (non-redundant)
Rest: 120s
═══
## 5) 🪫 Release
Subcode: ⛽🐬➖🟢 (Release | Ultra | Partner | Bodyweight)
├─ 6 ➖ Shot Put Throw (Single-Arm) (smooth tempo, downshift)
│  Set 1: ⛽ 60% × 6 (deload)
Rest: 120s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → ⛽🐬➖🟢 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Bodyweight strength is the transfer test. If it held, the gym work is real.
