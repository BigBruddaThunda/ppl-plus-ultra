---
zip: ⛽🐬➖🟠
operator: 🦉 logos
status: GENERATED
deck: 12
order: ⛽ Strength | 75-85% | 4-6 reps | 3-4 min | CNS: High
axis: 🐬 Partner | Social training context. Spottable, alternating, teachable.
type: ➖ Ultra | Partner-context strength emphasis
color: 🟠 Circuit | Partner deployment logic
blocks: ♨️ ▶️ 🧈 🧩 🪫 🚂
---
# ➖ Single-Leg Hops Through Ladder — Ultra Circuit ➖

## Strength Partner — Ultra focus (Circuit) · 50-65 min

**CODE:** ⛽🐬➖🟠

> "Rotate stations. Keep the tissues trading off under heavy demand."

*Pre/Post Natal: Gentle circuit — station rotation with manageable transitions*
═══
## 1) ♨️ Warm-Up — 🦉 logos
Subcode: ⛽🐬➖🟠 (Warm-Up | Ultra | Partner | Circuit)
├─ 6 ➖ Partner Toss (Alternating) (station rotation, keep moving)
│  Set 1: ⛽ 60% × 6 (pattern prep)
Rest: 45s
═══
## 2) 🎱 ARAM — Circuit Loop
Subcode: ⛽🐬➖🟠 (ARAM | Ultra | Partner | Circuit)
┌─ 3 rounds, rotate through stations. No two adjacent stations same muscle group.
│  Station 1: ➖ Single-Leg Hops Through Ladder × 5 (station rotation, keep moving)
│  Station 2: ➖ Partner Toss (Alternating) × 5 (station rotation, keep moving)
│  Station 3: ➖ Plant and Cut (Single-Leg Deceleration) × 5 (station rotation, keep moving)
│  Station 4: ➖ Layup Approach (Single-Leg Power) × 5 (station rotation, keep moving)
│  Transition: 15s between stations
│  Round rest: 60s
Rest: 90s after final round
═══
## 3) 🪫 Release
Subcode: ⛽🐬➖🟠 (Release | Ultra | Partner | Circuit)
├─ 6 ➖ Turkish Get-Up (Full) (smooth tempo, downshift)
│  Set 1: ⛽ 60% × 6 (deload)
Rest: 60s
═══
## 4) 🚂 Junction
- Log: load, reps, and form break point.
- Next → ⛽🐬➖🟠 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Log station completion times. Consistency matters more than speed.
