---
zip: ⛽🔨➕🟠
operator: 🥨 tendo (extend, push limits, stretch)
status: GENERATED
deck: 08
order: ⛽ Strength | 75–85% | 4–6 reps | 3–4 min | CNS: High
axis: 🔨 Functional | Unilateral, standing, athletic-transfer movements.
type: ➕ Plus | Full-body power, core stability
color: 🟠 Circuit | Tier 0–3 | GOLD: No | Station-based timed rotation. No barbells. Loop logic required.
blocks: ♨️ → 🎱 → 🪫 → 🚂
---
# ➕ Farmer Carry and Slam Rotation — Plus Circuit ➕

## Strength Functional — Plus focus (Circuit) · 50-65 min

**CODE:** ⛽🔨➕🟠

> "Rotate stations. Keep the tissues trading off under heavy demand."

*Kettlebell Strength: Station rotation with implements — keep tissues trading under heavy demand*
═══
## 1) ♨️ Warm-Up — 🥨 tendo (extend, push limits, stretch)
Subcode: ⛽🔨➕🟠 (Warm-Up | Plus | Functional | Circuit)
├─ 6 ➕ Shot Put Throw (Single-Arm) (station rotation, keep moving)
│  Set 1: ⛽ 60% × 6 (pattern prep)
Rest: 45s
═══
## 2) 🎱 ARAM — Circuit Loop
Subcode: ⛽🔨➕🟠 (ARAM | Plus | Functional | Circuit)
┌─ 3 rounds, rotate through stations. No two adjacent stations same muscle group.
│  Station 1: ➕ Farmer Carry and Slam Rotation × 5 (station rotation, keep moving)
│  Station 2: ➕ Shot Put Throw (Single-Arm) × 5 (station rotation, keep moving)
│  Station 3: ➕ Single-Leg Romanian Deadlift × 5 (station rotation, keep moving)
│  Station 4: ➕ Half-Kneeling Pallof Press × 5 (station rotation, keep moving)
│  Transition: 15s between stations
│  Round rest: 60s
Rest: 90s after final round
═══
## 3) 🪫 Release
Subcode: ⛽🔨➕🟠 (Release | Plus | Functional | Circuit)
├─ 6 ➕ Single-Arm Dumbbell Row (Anti-Rotation) (smooth tempo, downshift)
│  Set 1: ⛽ 60% × 6 (deload)
Rest: 60s
═══
## 4) 🚂 Junction
- Log: load, reps, and form break point.
- Next → ⛽🔨➕🟠 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Log station completion times. Consistency matters more than speed.
