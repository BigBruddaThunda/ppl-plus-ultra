---
zip: ⛽🌹➖⚫
operator: 👀 specio
status: GENERATED
deck: 09
order: ⛽ Strength | 75-85% | 4-6 reps | 3-4 min | CNS: High
axis: 🌹 Aesthetic | Isolation, full ROM, mind-muscle connection. Feel over load.
type: ➖ Ultra | Cardiovascular system, conditioning
color: ⚫ Teaching
blocks: ♨️ ▶️ 🧈 🧩 🪫 🚂
---
# ➖ Sprint (10-30 meters) — Ultra Teaching ➖

## Strength Aesthetic — Ultra focus (Teaching) · 50-65 min

**CODE:** ⛽🌹➖⚫

> "Learn where the bar sits and how it moves before adding plates."

*Post-Rehab Strength: Teaches barbell setup, loading sequence, and safety positions for heavy compound lifts*
═══
## 1) ♨️ Warm-Up — 👀 specio
Subcode: ⛽🌹➖⚫ (Warm-Up | Ultra | Aesthetic | Teaching)
├─ 6 ➖ Sprint (30-60 meters) (coached, check form before adding load)
│  Set 1: ⛽ 60% × 6 (pattern prep)
Rest: 135s
═══
## 2) 🛠 Craft
Subcode: ⛽🌹➖⚫ (Craft | Ultra | Aesthetic | Teaching)
├─ 5 ➖ Sprint (60-100 meters) (coach: check grip width, elbow angle, foot pressure)
│  Set 1: ⛽ 60% × 5 (practice the pattern, not the load)
│  Set 2: ⛽ 65% × 5 (same cues, slightly heavier)
Rest: 180s
═══
## 3) 🧈 Bread & Butter
Subcode: ⛽🌹➖⚫ (Bread & Butter | Ultra | Aesthetic | Teaching)
├─ 5 ➖ Sprint (10-30 meters) (coached, check form before adding load)
│  Set 1: ⛽ 75% × 5 (build set)
│  Set 2: ⛽ 80% × 5 (working set)
│  Set 3: ⛽ 80% × 5 (repeat quality)
Rest: 270s
═══
## 4) 🧩 Supplemental
Subcode: ⛽🌹➖⚫ (Supplemental | Ultra | Aesthetic | Teaching)
├─ 6 ➖ Hill Sprint (Short) (full range, different angle)
│  Set 1: ⛽ 70% × 6 (support volume)
│  Set 2: ⛽ 70% × 6 (non-redundant)
Rest: 180s
═══
## 5) 🧬 Imprint
Subcode: ⛽🌹➖⚫ (Imprint | Ultra | Aesthetic | Teaching)
├─ 6 ➖ Tempo Run (Moderate Pace) (high rep, low load, neural memory)
│  Set 1: ⛽ 55% × 6 (lock the pattern)
Rest: 180s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → ⛽🌹➖⚫ — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Note the coaching cues that changed the movement. Build from those.
