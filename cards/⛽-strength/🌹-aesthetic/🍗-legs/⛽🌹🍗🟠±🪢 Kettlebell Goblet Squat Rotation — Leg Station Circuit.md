---
zip: ⛽🌹🍗🟠
operator: 🪢 plico
status: GENERATED
deck: 09
order: ⛽ Strength | 75-85% | 4-6 reps | 3-4 min | CNS: High
axis: 🌹 Aesthetic | Isolation, full ROM, mind-muscle connection. Feel over load.
type: 🍗 Legs | Quads, hamstrings, glutes, calves
color: 🟠 Circuit
blocks: ♨️ ▶️ 🧈 🎱 🪫 🚂
---
# 🍗 Single-Leg Jump Rope — Legs Circuit 🍗

## Strength Aesthetic — Legs focus (Circuit) · 50-65 min

**CODE:** ⛽🌹🍗🟠

> "Rotate stations. Keep the tissues trading off under heavy demand."

*Physique / Recomp: Muscle group circuit — each station hits a different angle on the target tissue*
═══
## 1) ♨️ Warm-Up — 🪢 plico
Subcode: ⛽🌹🍗🟠 (Warm-Up | Legs | Aesthetic | Circuit)
├─ 6 🍗 Box Jump (Calf Power) (station rotation, keep moving)
│  Set 1: ⛽ 60% × 6 (pattern prep)
Rest: 45s
═══
## 2) 🎱 ARAM — Circuit Loop
Subcode: ⛽🌹🍗🟠 (ARAM | Legs | Aesthetic | Circuit)
┌─ 3 rounds, rotate through stations. No two adjacent stations same muscle group.
│  Station 1: 🍗 Single-Leg Jump Rope × 5 (station rotation, keep moving)
│  Station 2: 🍗 Box Jump (Calf Power) × 5 (station rotation, keep moving)
│  Station 3: 🍗 Depth Drop (Reactive Calf) × 5 (station rotation, keep moving)
│  Station 4: 🍗 Jump Rope × 5 (station rotation, keep moving)
│  Transition: 15s between stations
│  Round rest: 60s
Rest: 90s after final round
═══
## 3) 🪫 Release
Subcode: ⛽🌹🍗🟠 (Release | Legs | Aesthetic | Circuit)
├─ 6 🍗 Double-Unders (Jump Rope) (smooth tempo, downshift)
│  Set 1: ⛽ 60% × 6 (deload)
Rest: 60s
═══
## 4) 🚂 Junction
- Log: load, reps, and form break point.
- Next → ⛽🌹🍗🟠 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Log station completion times. Consistency matters more than speed.
