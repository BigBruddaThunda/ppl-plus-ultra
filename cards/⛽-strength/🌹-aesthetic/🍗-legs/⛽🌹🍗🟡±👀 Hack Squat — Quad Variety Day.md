---
zip: ⛽🌹🍗🟡
operator: 👀 specio
status: GENERATED
deck: 09
order: ⛽ Strength | 75-85% | 4-6 reps | 3-4 min | CNS: High
axis: 🌹 Aesthetic | Isolation, full ROM, mind-muscle connection. Feel over load.
type: 🍗 Legs | Quads, hamstrings, glutes, calves
color: 🟡 Fun
blocks: ♨️ ▶️ 🧈 🧩 🪫 🚂
---

# 🍗 Hack Squat — Quad Variety Day 🍗

Strength aesthetic session | Quads, glutes, hamstrings, calves | 58-72 min

CODE: ⛽🌹🍗🟡

> "Own each heavy rep with full range so the target leg tissue carries the load."

═══════════════════════════════════════

## 1. ♨️ WARM-UP — 👀 specio

Sub-block: ♨️🍗🌹🟡 (Warm-Up | Legs | Aesthetic | Fun)

├─ 8 🍗 Bodyweight Squat (hips open, chest tall)
├─ 8 🍗 Reverse Lunge (knee tracks forward)
└─ 8 🍗 Glute Bridge (squeeze at top)

Rest: 60 sec between passes.

═══════════════════════════════════════

## 2. ▶️ PRIMER

Sub-block: ▶️🍗🌹🟡 (Primer | Legs | Aesthetic | Fun)

├─ Set 1: ⛽ 55% × 6 on Hack Squat Machine (find stance)
├─ Set 2: ⛽ 65% × 5 on Hack Squat Machine (steady descent)
└─ Set 3: ⛽ 72% × 4 on Hack Squat Machine (drive hard, no rush)

Rest: 2 min between sets.

═══════════════════════════════════════

## 3. 🧈 BREAD & BUTTER — 👀 specio

Sub-block: 🧈🍗🌹🟡 (Bread & Butter | Legs | Aesthetic | Fun)

├─ 5 🍗 Hack Squat Machine (deep range, strong finish)
│  ├─ Set 1: ⛽ 75% × 5
│  ├─ Set 2: ⛽ 78% × 5
│  ├─ Set 3: ⛽ 82% × 4
│  └─ Set 4: ⛽ 85% × 4
└─ 4 🍗 Narrow-Stance Leg Press (tension never drops)
   ├─ Set 1: ⛽ 76% × 4
   ├─ Set 2: ⛽ 78% × 4
   └─ Set 3: ⛽ 80% × 4

Rest: 3-4 min between sets.

═══════════════════════════════════════

## 4. 🧩 SUPPLEMENTAL

Sub-block: 🧩🍗🌹🟡 (Supplemental | Legs | Aesthetic | Fun)

├─ 5 🍗 Weighted Box Step-Up (Dumbbells) (drive through mid-foot)
│  ├─ Set 1: ⛽ 75% × 5
│  ├─ Set 2: ⛽ 78% × 4
│  └─ Set 3: ⛽ 80% × 4
└─ 5 🍗 Cable Leg Curl (Ankle Strap) (full stretch, full finish)
   ├─ Set 1: ⛽ 74% × 5
   ├─ Set 2: ⛽ 76% × 5
   └─ Set 3: ⛽ 78% × 4

Rest: 3 min between sets.

═══════════════════════════════════════

## 5. 🪫 RELEASE

Sub-block: 🪫🍗🌹🟡 (Release | Legs | Aesthetic | Fun)

- 60 sec easy nasal breathing with long exhale.

Rest: none.

═══════════════════════════════════════

## 6. 🚂 JUNCTION

Sub-block: 🚂🍗🌹🟡 (Junction | Legs | Aesthetic | Fun)

Next → ⛽🌹🛒🟡 — upper-body push next while legs recover.
Next → ⛽🌹🪡🟡 — pull day balances torso loading.
Next → ⛽🌹➖🟡 — engine work supports next leg session capacity.

Log: top load ______ | clean reps ______ | note ______

═══════════════════════════════════════

## 🧮 SAVE

Heavy aesthetic leg work is controlled force: if depth and line stay consistent, progression is earned. If range shortens, keep load and repeat.
