---
zip: ⛽🏛🛒⚪
operator: 📍 pono
status: GENERATED
deck: 07
updated: 2026-03-06
order: ⛽ Strength | 75–85% | 4–6 reps | 3–4 min | CNS: High
axis: 🏛 Basics | Bilateral, barbell-first, proven classics
type: 🛒 Push | Chest, front delts, triceps
color: ⚪ Mindful | Tier 0–3 | GOLD: No | 4-second eccentrics. Extended rest. Breath.
blocks: ♨️ → ▶️ → 🧈 → 🪫 → 🚂 → 🧮
---
# 🛒 Kettlebell Swing (Two-Hand) — Push Mindful 🛒

## Strength Basics — Push focus (Mindful) · 50-65 min

**CODE:** ⛽🏛🛒⚪

> "Four seconds down. Feel every degree of the range before you earn the drive."

*Marathon / Distance Running: Zone 2 steady state — conversational pace, extended duration, aerobic base*
═══
## 1) ♨️ Warm-Up — 📍 pono
Subcode: ⛽🏛🛒⚪ (Warm-Up | Push | Basics | Mindful)
├─ 6 🛒 American Swing (Overhead) (4s eccentric, breath-paced)
│  Set 1: ⛽ 60% × 6 (pattern prep)
Rest: 180s
═══
## 2) 🧈 Bread & Butter
Subcode: ⛽🏛🛒⚪ (Bread & Butter | Push | Basics | Mindful)
├─ 5 🛒 Kettlebell Swing (Two-Hand) (4s eccentric, breath-paced)
│  Set 1: ⛽ 75% × 5 (build set)
│  Set 2: ⛽ 80% × 5 (working set)
│  Set 3: ⛽ 80% × 5 (repeat quality)
Rest: 360s
═══
## 3) 🧩 Supplemental
Subcode: ⛽🏛🛒⚪ (Supplemental | Push | Basics | Mindful)
├─ 6 🛒 Dead Stop Swing (Reset Between Reps) (full range, different angle)
│  Set 1: ⛽ 70% × 6 (support volume)
│  Set 2: ⛽ 70% × 6 (non-redundant)
Rest: 240s
═══
## 4) 🪫 Release
Subcode: ⛽🏛🛒⚪ (Release | Push | Basics | Mindful)
├─ 6 🛒 Swing to High Pull (parasympathetic, tension down, 4s eccentric)
│  Set 1: ⛽ 60% × 6 (deload)
Rest: 240s
═══
## 5) 🧬 Imprint
Subcode: ⛽🏛🛒⚪ (Imprint | Push | Basics | Mindful)
├─ 6 🛒 Swing + Clean (Combo) (high rep, low load, neural memory)
│  Set 1: ⛽ 55% × 6 (lock the pattern)
Rest: 240s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → ⛽🏛🛒⚪ — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Slow strength builds tendons, not just muscles. Trust the tempo.
