---
zip: ⛽⌛➕🔵
operator: ✒️ grapho
status: GENERATED
deck: 11
order: ⛽ Strength | 75-85% | 4-6 reps | 3-4 min | CNS: High
axis: ⌛ Time | Clock as training variable. EMOM, AMRAP, density, timed sets.
type: ➕ Plus | Full body power and core
color: 🔵 Structured | Tier 0-5 | GOLD: No | Clock-governed strength pacing.
blocks: ♨️ ▶️ 🧈 🧩 🪫 🚂
---
# ➕ Trap Bar Carry Intervals — Plus Structured ➕

## Strength Time — Plus focus (Structured) · 50-65 min

**CODE:** ⛽⌛➕🔵

> "Same weight, same reps, same rest. Build the base one session at a time."
═══
## 1) ♨️ Warm-Up — ✒️ grapho
Subcode: ⛽⌛➕🔵 (Warm-Up | Plus | Time | Structured)
├─ 6 ➕ Pull-Up (Overhand Grip) (prescribed, track sets and reps)
│  Set 1: ⛽ 60% × 6 (pattern prep)
Rest: 90s
═══
## 2) ▶️ Primer
Subcode: ⛽⌛➕🔵 (Primer | Plus | Time | Structured)
├─ 5 ➕ Bent-Over Barbell Row (Overhand) (tight setup, crisp intent)
│  Set 1: ⛽ 70% × 5 (activation)
Rest: 120s
═══
## 3) 🧈 Bread & Butter
Subcode: ⛽⌛➕🔵 (Bread & Butter | Plus | Time | Structured)
├─ 5 ➕ Trap Bar Carry Intervals (prescribed, track sets and reps)
│  Set 1: ⛽ 75% × 5 (build set)
│  Set 2: ⛽ 80% × 5 (working set)
│  Set 3: ⛽ 80% × 5 (repeat quality)
Rest: 180s
═══
## 4) 🧩 Supplemental
Subcode: ⛽⌛➕🔵 (Supplemental | Plus | Time | Structured)
├─ 6 ➕ Chest-Supported Row (Elbow Out) (full range, different angle)
│  Set 1: ⛽ 70% × 6 (support volume)
│  Set 2: ⛽ 70% × 6 (non-redundant)
Rest: 120s
═══
## 5) 🪫 Release
Subcode: ⛽⌛➕🔵 (Release | Plus | Time | Structured)
├─ 6 ➕ Deadlift (Conventional) (smooth tempo, downshift)
│  Set 1: ⛽ 60% × 6 (deload)
Rest: 120s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → ⛽⌛➕🔵 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Keep the same movement standard. Only add load if bar path stays unchanged.
