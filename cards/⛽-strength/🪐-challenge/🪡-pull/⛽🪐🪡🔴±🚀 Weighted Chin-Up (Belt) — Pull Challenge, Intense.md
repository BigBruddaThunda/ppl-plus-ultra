---
zip: ⛽🪐🪡🔴
operator: 🚀 mitto
status: GENERATED
deck: 10
order: ⛽ Strength | 75-85% | 4-6 reps | 3-4 min | CNS: High
axis: 🪐 Challenge | Hardest variation at any level. Deficit, pause, tempo, bands.
type: 🪡 Pull
color: 🔴 Intense
blocks: ♨️ ▶️ 🧈 🧩 🪫 🚂
---
# 🪡 Alternating Dumbbell Front Raise — Pull Intense 🪡

## Strength Challenge — Pull focus (Intense) · 50-65 min

**CODE:** ⛽🪐🪡🔴

> "Push the ceiling. Every set should cost something real."

*Physique / Recomp: Chase the pump — high volume, low rest, supersets allowed, metabolic stress*
═══
## 1) ♨️ Warm-Up — 🚀 mitto
Subcode: ⛽🪐🪡🔴 (Warm-Up | Pull | Challenge | Intense)
├─ 6 🪡 Single-Arm Dumbbell Lateral Raise (high effort, push the pace)
│  Set 1: ⛽ 60% × 6 (pattern prep)
Rest: 45s
═══
## 2) ▶️ Primer
Subcode: ⛽🪐🪡🔴 (Primer | Pull | Challenge | Intense)
├─ 5 🪡 Single-Arm Cable Rear Delt Fly (tight setup, crisp intent)
│  Set 1: ⛽ 70% × 5 (activation)
Rest: 60s
═══
## 3) 🧈 Bread & Butter
Subcode: ⛽🪐🪡🔴 (Bread & Butter | Pull | Challenge | Intense)
├─ 5 🪡 Alternating Dumbbell Front Raise (high effort, push the pace)
│  Set 1: ⛽ 75% × 5 (build set)
│  Set 2: ⛽ 80% × 5 (working set)
│  Set 3: ⛽ 80% × 5 (repeat quality)
Rest: 90s
═══
## 4) 🧩 Supplemental
Subcode: ⛽🪐🪡🔴 (Supplemental | Pull | Challenge | Intense)
├─ 6 🪡 Single-Arm Overhead Dumbbell Hold (full range, different angle)
│  Set 1: ⛽ 70% × 6 (support volume)
│  Set 2: ⛽ 70% × 6 (non-redundant)
Rest: 60s
═══
## 5) 🌋 Gutter
Subcode: ⛽🪐🪡🔴 (Gutter | Pull | Challenge | Intense)
├─ 6 🪡 Single-Arm Face Pull (all-out, leave nothing in reserve)
│  Set 1: ⛽ 65% × 6 (push to form break)
│  Set 2: ⛽ 65% × AMRAP (final effort)
Rest: 45s
═══
## 6) 🪫 Release
Subcode: ⛽🪐🪡🔴 (Release | Pull | Challenge | Intense)
├─ 6 🪡 Single-Arm Face Pull (stress out, cathartic discharge)
│  Set 1: ⛽ 60% × 6 (deload)
Rest: 60s
═══
## 7) 🚂 Junction
- Log: load, reps, and form break point.
- Next → ⛽🪐🪡🔴 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Record peak effort. Recovery before next heavy session.
