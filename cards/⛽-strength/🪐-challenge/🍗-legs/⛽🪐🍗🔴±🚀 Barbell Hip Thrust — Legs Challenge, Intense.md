---
zip: ⛽🪐🍗🔴
operator: 🚀 mitto
status: GENERATED
deck: 10
order: ⛽ Strength | 75-85% | 4-6 reps | 3-4 min | CNS: High
axis: 🪐 Challenge | Hardest variation at any level. Deficit, pause, tempo, bands.
type: 🍗 Legs
color: 🔴 Intense
blocks: ♨️ ▶️ 🧈 🧩 🪫 🚂
---
# 🍗 Kettlebell Swing (Single-Arm) — Legs Intense 🍗

## Strength Challenge — Legs focus (Intense) · 50-65 min

**CODE:** ⛽🪐🍗🔴

> "Push the ceiling. Every set should cost something real."

*Sprint / Power Conditioning: All-out intervals — max effort, full recovery, repeat until capacity drops*
═══
## 1) ♨️ Warm-Up — 🚀 mitto
Subcode: ⛽🪐🍗🔴 (Warm-Up | Legs | Challenge | Intense)
├─ 6 🍗 Kettlebell Swing (Alternating Arms) (high effort, push the pace)
│  Set 1: ⛽ 60% × 6 (pattern prep)
Rest: 45s
═══
## 2) ▶️ Primer
Subcode: ⛽🪐🍗🔴 (Primer | Legs | Challenge | Intense)
├─ 5 🍗 Single-Arm American Swing (tight setup, crisp intent)
│  Set 1: ⛽ 70% × 5 (activation)
Rest: 60s
═══
## 3) 🧈 Bread & Butter
Subcode: ⛽🪐🍗🔴 (Bread & Butter | Legs | Challenge | Intense)
├─ 5 🍗 Kettlebell Swing (Single-Arm) (high effort, push the pace)
│  Set 1: ⛽ 75% × 5 (build set)
│  Set 2: ⛽ 80% × 5 (working set)
│  Set 3: ⛽ 80% × 5 (repeat quality)
Rest: 90s
═══
## 4) 🧩 Supplemental
Subcode: ⛽🪐🍗🔴 (Supplemental | Legs | Challenge | Intense)
├─ 6 🍗 Single-Leg Swing (full range, different angle)
│  Set 1: ⛽ 70% × 6 (support volume)
│  Set 2: ⛽ 70% × 6 (non-redundant)
Rest: 60s
═══
## 5) 🌋 Gutter
Subcode: ⛽🪐🍗🔴 (Gutter | Legs | Challenge | Intense)
├─ 6 🍗 Alternating Kettlebell Snatch (all-out, leave nothing in reserve)
│  Set 1: ⛽ 65% × 6 (push to form break)
│  Set 2: ⛽ 65% × AMRAP (final effort)
Rest: 45s
═══
## 6) 🪫 Release
Subcode: ⛽🪐🍗🔴 (Release | Legs | Challenge | Intense)
├─ 6 🍗 Alternating Kettlebell Snatch (stress out, cathartic discharge)
│  Set 1: ⛽ 60% × 6 (deload)
Rest: 60s
═══
## 7) 🚂 Junction
- Log: load, reps, and form break point.
- Next → ⛽🪐🍗🔴 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Record peak effort. Recovery before next heavy session.
