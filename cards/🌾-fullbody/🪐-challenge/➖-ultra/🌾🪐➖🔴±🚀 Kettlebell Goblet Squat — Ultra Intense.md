---
zip: 🌾🪐➖🔴
operator: 🚀 mitto
status: GENERATED
deck: 28
order: 🌾 Full Body | ~70% | 8-10 reps | 30-90s | CNS: Moderate
axis: 🪐 Challenge | Hardest variation at any level. Deficit, pause, tempo, bands.
type: ➖ Ultra | Cardiovascular system
color: 🔴 Intense | Tier 2-4 | GOLD: Yes | Maximum effort. High volume. Reduced rest. Supersets OK.
blocks: ♨️ 🎼 🧈 🧩 🪫 🚂
---
# ➖ Turkish Get-Up (Full) — Ultra Intense ➖

## Full Body Challenge — Ultra focus (Intense) · 40-55 min

**CODE:** 🌾🪐➖🔴

> "Drive through the integrated pattern with intent. No wasted motion."

*Bodybuilding: Chase the pump — high volume, low rest, supersets allowed, metabolic stress*
═══
## 1) ♨️ Warm-Up — 🚀 mitto
Subcode: 🌾🪐➖🔴 (Warm-Up | Ultra | Challenge | Intense)
├─ 10 ➖ Half Turkish Get-Up (To Elbow) (high effort, push the pace)
│  Set 1: 🌾 50% × 10 (pattern prep)
Rest: 30s
═══
## 2) ▶️ Primer
Subcode: 🌾🪐➖🔴 (Primer | Ultra | Challenge | Intense)
├─ 9 ➖ Turkish Get-Up (To Seated) (tight setup, crisp intent)
│  Set 1: 🌾 60% × 9 (activation)
Rest: 30s
═══
## 3) 🧈 Bread & Butter
Subcode: 🌾🪐➖🔴 (Bread & Butter | Ultra | Challenge | Intense)
├─ 9 ➖ Turkish Get-Up (Full) (high effort, push the pace)
│  Set 1: 🌾 65% × 9 (build set)
│  Set 2: 🌾 70% × 9 (working set)
│  Set 3: 🌾 70% × 9 (repeat quality)
Rest: 30s
═══
## 4) 🧩 Supplemental
Subcode: 🌾🪐➖🔴 (Supplemental | Ultra | Challenge | Intense)
├─ 10 ➖ Turkish Get-Up (To Kneeling) (full range, different angle)
│  Set 1: 🌾 60% × 10 (support volume)
│  Set 2: 🌾 60% × 10 (non-redundant)
Rest: 30s
═══
## 5) 🌋 Gutter
Subcode: 🌾🪐➖🔴 (Gutter | Ultra | Challenge | Intense)
├─ 10 ➖ Bottoms-Up Turkish Get-Up (all-out, leave nothing in reserve)
│  Set 1: 🌾 55% × 10 (push to form break)
│  Set 2: 🌾 55% × AMRAP (final effort)
Rest: 45s
═══
## 6) 🪫 Release
Subcode: 🌾🪐➖🔴 (Release | Ultra | Challenge | Intense)
├─ 10 ➖ Bottoms-Up Turkish Get-Up (stress out, cathartic discharge)
│  Set 1: 🌾 50% × 10 (deload)
Rest: 30s
═══
## 7) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🌾🪐➖🔴 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Integrated effort under fatigue reveals the real pattern. Log the breakdown point.
