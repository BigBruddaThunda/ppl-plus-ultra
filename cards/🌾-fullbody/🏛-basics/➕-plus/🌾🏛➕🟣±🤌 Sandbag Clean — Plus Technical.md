---
zip: 🌾🏛➕🟣
operator: 🤌 facio
status: GENERATED
deck: 25
order: 🌾 Full Body | ~70% | 8-10 reps | 30-90s | CNS: Moderate
axis: 🏛 Basics | Bilateral, stable, barbell-first classics. Compound over isolation.
type: ➕ Plus | Full body power, core
color: 🟣 Technical | Tier 2-5 | GOLD: Yes | Precision. Lower volume, extended rest, quality focus.
blocks: ♨️ 🎼 🧈 🧩 🪫 🚂
---
# ➕ Sandbag Clean — Plus Technical ➕

## Full Body Basics — Plus focus (Technical) · 40-55 min

**CODE:** 🌾🏛➕🟣

> "Precision in the transitions. The connection between movements is the skill."
═══
## 1) ♨️ Warm-Up — 🤌 facio
Subcode: 🌾🏛➕🟣 (Warm-Up | Plus | Basics | Technical)
├─ 10 ➕ Atlas Stone Carry (precision, quality over volume)
│  Set 1: 🌾 50% × 10 (pattern prep)
Rest: 90s
═══
## 2) ▶️ Primer
Subcode: 🌾🏛➕🟣 (Primer | Plus | Basics | Technical)
├─ 9 ➕ Car Deadlift (Straps) (tight setup, crisp intent)
│  Set 1: 🌾 60% × 9 (activation)
Rest: 90s
═══
## 3) 🧈 Bread & Butter
Subcode: 🌾🏛➕🟣 (Bread & Butter | Plus | Basics | Technical)
├─ 9 ➕ Sandbag Clean (precision, quality over volume)
│  Set 1: 🌾 65% × 9 (build set)
│  Set 2: 🌾 70% × 9 (working set)
│  Set 3: 🌾 70% × 9 (repeat quality)
Rest: 90s
═══
## 4) 🧩 Supplemental
Subcode: 🌾🏛➕🟣 (Supplemental | Plus | Basics | Technical)
├─ 10 ➕ Push Jerk (full range, different angle)
│  Set 1: 🌾 60% × 10 (support volume)
│  Set 2: 🌾 60% × 10 (non-redundant)
Rest: 67s
═══
## 5) 🪫 Release
Subcode: 🌾🏛➕🟣 (Release | Plus | Basics | Technical)
├─ 10 ➕ Squat Jump (No Countermovement) (smooth tempo, downshift)
│  Set 1: 🌾 50% × 10 (deload)
Rest: 67s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🌾🏛➕🟣 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Note the weakest transition. That is where the chain needs work.
