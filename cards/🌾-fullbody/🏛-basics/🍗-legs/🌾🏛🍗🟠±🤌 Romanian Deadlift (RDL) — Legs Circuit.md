---
zip: 🌾🏛🍗🟠
operator: 🤌 facio
status: GENERATED
deck: 25
order: 🌾 Full Body | ~70% | 8-10 reps | 30-90s | CNS: Moderate
axis: 🏛 Basics | Bilateral, stable, barbell-first classics. Compound over isolation.
type: 🍗 Legs | Quads, hamstrings, glutes, calves
color: 🟠 Circuit | Tier 0-3 | GOLD: No | Station-based timed rotation. No barbells. Loop logic required.
blocks: ♨️ 🎼 🧈 🧩 🪫 🚂
---
# 🍗 Romanian Deadlift (RDL) — Legs Circuit 🍗

## Full Body Basics — Legs focus (Circuit) · 40-55 min

**CODE:** 🌾🏛🍗🟠

> "Flow through the full body as one integrated pattern."
═══
## 1) ♨️ Warm-Up — 🤌 facio
Subcode: 🌾🏛🍗🟠 (Warm-Up | Legs | Basics | Circuit)
├─ 10 🍗 Deep Squat Hold (Ankle Mobility) (station rotation, keep moving)
│  Set 1: 🌾 50% × 10 (pattern prep)
Rest: 60s
═══
## 2) 🎼 Composition
Subcode: 🌾🏛🍗🟠 (Composition | Legs | Basics | Circuit)
├─ 9 🍗 Leg Press Calf Raise → Romanian Deadlift (RDL) (flow without reset)
│  Set 1: 🌾 70% × 9 (unified pattern)
│  Set 2: 🌾 70% × 9 (repeat flow)
Rest: 60s
═══
## 3) 🎱 ARAM — Circuit Loop
Subcode: 🌾🏛🍗🟠 (ARAM | Legs | Basics | Circuit)
┌─ 3 rounds, rotate through stations. No two adjacent stations same muscle group.
│  Station 1: 🍗 Romanian Deadlift (RDL) × 9 (station rotation, keep moving)
│  Station 2: 🍗 Deep Squat Hold (Ankle Mobility) × 9 (station rotation, keep moving)
│  Station 3: 🍗 Leg Press Calf Raise × 9 (station rotation, keep moving)
│  Station 4: 🍗 Walking Lunge × 9 (station rotation, keep moving)
│  Transition: 15s between stations
│  Round rest: 60s
Rest: 90s after final round
═══
## 5) 🪫 Release
Subcode: 🌾🏛🍗🟠 (Release | Legs | Basics | Circuit)
├─ 12 🍗 Spanish Squat (Banded VMO) (smooth tempo, downshift)
│  Set 1: 🌾 50% × 12 (deload)
Rest: 45s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🌾🏛🍗🟠 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
The flow is the measure. If movements disconnected, simplify next session.
