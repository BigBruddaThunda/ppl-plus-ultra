---
zip: 🌾🔨🪡🟣
operator: 🥨 tendo
status: GENERATED
deck: 26
order: 🌾 Full Body | ~70% | 8-10 reps | 30-90s | CNS: Moderate
axis: 🔨 Functional | Unilateral, standing, athletic-transfer movements.
type: 🪡 Pull | Lats, rear delts, biceps, traps, erectors
color: 🟣 Technical | Tier 2-5 | GOLD: Yes | Precision. Lower volume, extended rest, quality focus.
blocks: ♨️ 🎼 🧈 🧩 🪫 🚂
---
# 🪡 Keg Carry — Pull Technical 🪡

## Full Body Functional — Pull focus (Technical) · 40-55 min

**CODE:** 🌾🔨🪡🟣

> "Flow through the full body as one integrated pattern."
═══
## 1) ♨️ Warm-Up — 🥨 tendo
Subcode: 🌾🔨🪡🟣 (Warm-Up | Pull | Functional | Technical)
├─ 10 🪡 Zercher Carry (precision, quality over volume)
│  Set 1: 🌾 50% × 10 (pattern prep)
Rest: 60s
═══
## 2) 🎼 Composition
Subcode: 🌾🔨🪡🟣 (Composition | Pull | Functional | Technical)
├─ 9 🪡 Single-Leg Romanian Deadlift → Keg Carry (flow without reset)
│  Set 1: 🌾 70% × 9 (unified pattern)
│  Set 2: 🌾 70% × 9 (repeat flow)
Rest: 60s
═══
## 3) 🧈 Bread & Butter
Subcode: 🌾🔨🪡🟣 (Bread & Butter | Pull | Functional | Technical)
├─ 9 🪡 Keg Carry (precision, quality over volume)
│  Set 1: 🌾 65% × 9 (build set)
│  Set 2: 🌾 70% × 9 (working set)
│  Set 3: 🌾 70% × 9 (repeat quality)
Rest: 60s
═══
## 4) 🧩 Supplemental
Subcode: 🌾🔨🪡🟣 (Supplemental | Pull | Functional | Technical)
├─ 11 🪡 Single-Arm Face Pull (full range, different angle)
│  Set 1: 🌾 60% × 11 (support volume)
│  Set 2: 🌾 60% × 11 (non-redundant)
Rest: 45s
═══
## 5) 🪫 Release
Subcode: 🌾🔨🪡🟣 (Release | Pull | Functional | Technical)
├─ 12 🪡 Archer Pull-Up (smooth tempo, downshift)
│  Set 1: 🌾 50% × 12 (deload)
Rest: 45s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🌾🔨🪡🟣 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
The flow is the measure. If movements disconnected, simplify next session.
