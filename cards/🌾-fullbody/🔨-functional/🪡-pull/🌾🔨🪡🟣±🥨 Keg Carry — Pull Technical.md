---
zip: 🌾🔨🪡🟣
operator: 🥨 tendo
status: GENERATED
deck: 26
order: 🌾 Full Body | ~70% | 8-10 reps | 30-90s | CNS: Moderate
axis: 🔨 Functional | Unilateral, standing, athletic-transfer movements.
type: 🪡 Pull | Lats, rear delts, biceps, traps, erectors
color: 🟣 Technical | Tier 2-5 | GOLD: Yes | Precision. Lower volume, extended rest, quality focus.
blocks: ♨️ 🎼 🧈 🧩 🪫 🚂
---
# 🪡 Kettlebell Swing (Single-Arm) — Pull Technical 🪡

## Full Body Functional — Pull focus (Technical) · 40-55 min

**CODE:** 🌾🔨🪡🟣

> "Precision in the transitions. The connection between movements is the skill."

*Sport Conditioning: Technical conditioning — rowing stroke, cycling cadence, running gait precision*
═══
## 1) ♨️ Warm-Up — 🥨 tendo
Subcode: 🌾🔨🪡🟣 (Warm-Up | Pull | Functional | Technical)
├─ 10 🪡 Kettlebell Swing (Alternating Arms) (precision, quality over volume)
│  Set 1: 🌾 50% × 10 (pattern prep)
Rest: 90s
═══
## 2) ▶️ Primer
Subcode: 🌾🔨🪡🟣 (Primer | Pull | Functional | Technical)
├─ 9 🪡 Single-Arm American Swing (tight setup, crisp intent)
│  Set 1: 🌾 60% × 9 (activation)
Rest: 90s
═══
## 3) 🧈 Bread & Butter
Subcode: 🌾🔨🪡🟣 (Bread & Butter | Pull | Functional | Technical)
├─ 9 🪡 Kettlebell Swing (Single-Arm) (precision, quality over volume)
│  Set 1: 🌾 65% × 9 (build set)
│  Set 2: 🌾 70% × 9 (working set)
│  Set 3: 🌾 70% × 9 (repeat quality)
Rest: 90s
═══
## 4) 🧩 Supplemental
Subcode: 🌾🔨🪡🟣 (Supplemental | Pull | Functional | Technical)
├─ 10 🪡 Single-Leg Swing (full range, different angle)
│  Set 1: 🌾 60% × 10 (support volume)
│  Set 2: 🌾 60% × 10 (non-redundant)
Rest: 67s
═══
## 5) 🪫 Release
Subcode: 🌾🔨🪡🟣 (Release | Pull | Functional | Technical)
├─ 10 🪡 Suitcase Carry (Single-Arm) (smooth tempo, downshift)
│  Set 1: 🌾 50% × 10 (deload)
Rest: 67s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🌾🔨🪡🟣 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Note the weakest transition. That is where the chain needs work.
