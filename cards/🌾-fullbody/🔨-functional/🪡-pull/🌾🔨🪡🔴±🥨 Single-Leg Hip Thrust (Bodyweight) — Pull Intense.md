---
zip: 🌾🔨🪡🔴
operator: 🥨 tendo
status: GENERATED
deck: 26
order: 🌾 Full Body | ~70% | 8-10 reps | 30-90s | CNS: Moderate
axis: 🔨 Functional | Unilateral, standing, athletic-transfer movements.
type: 🪡 Pull | Lats, rear delts, biceps, traps, erectors
color: 🔴 Intense | Tier 2-4 | GOLD: Yes | Maximum effort. High volume. Reduced rest. Supersets OK.
blocks: ♨️ 🎼 🧈 🧩 🪫 🚂
---
# 🪡 Single-Arm Cable Rear Delt Fly — Pull Intense 🪡

## Full Body Functional — Pull focus (Intense) · 40-55 min

**CODE:** 🌾🔨🪡🔴

> "Drive through the integrated pattern with intent. No wasted motion."

*Senior Strength: Heavy working sets with full recovery — push the ceiling, earn every rep*
═══
## 1) ♨️ Warm-Up — 🥨 tendo
Subcode: 🌾🔨🪡🔴 (Warm-Up | Pull | Functional | Intense)
├─ 10 🪡 Single-Arm Overhead Dumbbell Hold (high effort, push the pace)
│  Set 1: 🌾 50% × 10 (pattern prep)
Rest: 30s
═══
## 2) ▶️ Primer
Subcode: 🌾🔨🪡🔴 (Primer | Pull | Functional | Intense)
├─ 9 🪡 Single-Arm Face Pull (tight setup, crisp intent)
│  Set 1: 🌾 60% × 9 (activation)
Rest: 30s
═══
## 3) 🧈 Bread & Butter
Subcode: 🌾🔨🪡🔴 (Bread & Butter | Pull | Functional | Intense)
├─ 9 🪡 Single-Arm Cable Rear Delt Fly (high effort, push the pace)
│  Set 1: 🌾 65% × 9 (build set)
│  Set 2: 🌾 70% × 9 (working set)
│  Set 3: 🌾 70% × 9 (repeat quality)
Rest: 30s
═══
## 4) 🧩 Supplemental
Subcode: 🌾🔨🪡🔴 (Supplemental | Pull | Functional | Intense)
├─ 10 🪡 Aquaman (Alternating Superman) (full range, different angle)
│  Set 1: 🌾 60% × 10 (support volume)
│  Set 2: 🌾 60% × 10 (non-redundant)
Rest: 30s
═══
## 5) 🌋 Gutter
Subcode: 🌾🔨🪡🔴 (Gutter | Pull | Functional | Intense)
├─ 10 🪡 Half-Kneeling Band Single-Arm Pulldown (all-out, leave nothing in reserve)
│  Set 1: 🌾 55% × 10 (push to form break)
│  Set 2: 🌾 55% × AMRAP (final effort)
Rest: 45s
═══
## 6) 🪫 Release
Subcode: 🌾🔨🪡🔴 (Release | Pull | Functional | Intense)
├─ 10 🪡 Half-Kneeling Band Single-Arm Pulldown (stress out, cathartic discharge)
│  Set 1: 🌾 50% × 10 (deload)
Rest: 30s
═══
## 7) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🌾🔨🪡🔴 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Integrated effort under fatigue reveals the real pattern. Log the breakdown point.
