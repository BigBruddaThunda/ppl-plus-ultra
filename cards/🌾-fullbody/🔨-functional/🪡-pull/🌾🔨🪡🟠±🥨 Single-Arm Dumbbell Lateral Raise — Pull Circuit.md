---
zip: 🌾🔨🪡🟠
operator: 🥨 tendo
status: GENERATED
deck: 26
order: 🌾 Full Body | ~70% | 8-10 reps | 30-90s | CNS: Moderate
axis: 🔨 Functional | Unilateral, standing, athletic-transfer movements.
type: 🪡 Pull | Lats, rear delts, biceps, traps, erectors
color: 🟠 Circuit | Tier 0-3 | GOLD: No | Station-based timed rotation. No barbells. Loop logic required.
blocks: ♨️ 🎼 🧈 🧩 🪫 🚂
---
# 🪡 Kettlebell Swing (Single-Arm) — Pull Circuit 🪡

## Full Body Functional — Pull focus (Circuit) · 40-55 min

**CODE:** 🌾🔨🪡🟠

> "Rotate through full-body stations. Every station moves a different chain."

*Sprint / Power Conditioning: Mixed-modality circuit — each station changes the energy demand*
═══
## 1) ♨️ Warm-Up — 🥨 tendo
Subcode: 🌾🔨🪡🟠 (Warm-Up | Pull | Functional | Circuit)
├─ 10 🪡 Kettlebell Swing (Alternating Arms) (station rotation, keep moving)
│  Set 1: 🌾 50% × 10 (pattern prep)
Rest: 30s
═══
## 2) 🎱 ARAM — Circuit Loop
Subcode: 🌾🔨🪡🟠 (ARAM | Pull | Functional | Circuit)
┌─ 3 rounds, rotate through stations. No two adjacent stations same muscle group.
│  Station 1: 🪡 Kettlebell Swing (Single-Arm) × 9 (station rotation, keep moving)
│  Station 2: 🪡 Kettlebell Swing (Alternating Arms) × 9 (station rotation, keep moving)
│  Station 3: 🪡 Single-Arm American Swing × 9 (station rotation, keep moving)
│  Station 4: 🪡 Single-Leg Swing × 9 (station rotation, keep moving)
│  Transition: 15s between stations
│  Round rest: 30s
Rest: 90s after final round
═══
## 3) 🪫 Release
Subcode: 🌾🔨🪡🟠 (Release | Pull | Functional | Circuit)
├─ 10 🪡 Suitcase Carry (Single-Arm) (smooth tempo, downshift)
│  Set 1: 🌾 50% × 10 (deload)
Rest: 30s
═══
## 4) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🌾🔨🪡🟠 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Full-body circuit hits every chain. Note which station caused the most fatigue.
