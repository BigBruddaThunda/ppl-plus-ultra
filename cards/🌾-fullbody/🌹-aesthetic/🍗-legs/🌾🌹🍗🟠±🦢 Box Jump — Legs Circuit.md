---
zip: 🌾🌹🍗🟠
operator: 🦢 plico
status: GENERATED
deck: 27
order: 🌾 Full Body | ~70% | 8-10 reps | 30-90s | CNS: Moderate
axis: 🌹 Aesthetic | Isolation, full ROM, mind-muscle connection. Feel over load.
type: 🍗 Legs | Quads, hamstrings, glutes, calves
color: 🟠 Circuit | Tier 0-3 | GOLD: No | Station-based timed rotation. No barbells. Loop logic required.
blocks: ♨️ 🎼 🧈 🧩 🪫 🚂
---
# 🍗 Box Jump — Legs Circuit 🍗

## Full Body Aesthetic — Legs focus (Circuit) · 40-55 min

**CODE:** 🌾🌹🍗🟠

> "Flow through the full body as one integrated pattern."
═══
## 1) ♨️ Warm-Up — 🦢 plico
Subcode: 🌾🌹🍗🟠 (Warm-Up | Legs | Aesthetic | Circuit)
├─ 10 🍗 Half-Kneeling Hip Flexor Stretch (station rotation, keep moving)
│  Set 1: 🌾 50% × 10 (pattern prep)
Rest: 60s
═══
## 2) 🎼 Composition
Subcode: 🌾🌹🍗🟠 (Composition | Legs | Aesthetic | Circuit)
├─ 9 🍗 Single-Leg Extension → Box Jump (flow without reset)
│  Set 1: 🌾 70% × 9 (unified pattern)
│  Set 2: 🌾 70% × 9 (repeat flow)
Rest: 60s
═══
## 3) 🎱 ARAM — Circuit Loop
Subcode: 🌾🌹🍗🟠 (ARAM | Legs | Aesthetic | Circuit)
┌─ 3 rounds, rotate through stations. No two adjacent stations same muscle group.
│  Station 1: 🍗 Box Jump × 9 (station rotation, keep moving)
│  Station 2: 🍗 Half-Kneeling Hip Flexor Stretch × 9 (station rotation, keep moving)
│  Station 3: 🍗 Single-Leg Extension × 9 (station rotation, keep moving)
│  Station 4: 🍗 Single-Leg Good Morning × 9 (station rotation, keep moving)
│  Transition: 15s between stations
│  Round rest: 60s
Rest: 90s after final round
═══
## 5) 🪫 Release
Subcode: 🌾🌹🍗🟠 (Release | Legs | Aesthetic | Circuit)
├─ 12 🍗 Single-Leg Curl (Lying) (smooth tempo, downshift)
│  Set 1: 🌾 50% × 12 (deload)
Rest: 45s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🌾🌹🍗🟠 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
The flow is the measure. If movements disconnected, simplify next session.
