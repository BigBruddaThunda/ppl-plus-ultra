---
zip: 🌾🌹🪡🟣
operator: 🦢 plico
status: GENERATED
deck: 27
order: 🌾 Full Body | ~70% | 8-10 reps | 30-90s | CNS: Moderate
axis: 🌹 Aesthetic | Isolation, full ROM, mind-muscle connection. Feel over load.
type: 🪡 Pull | Lats, rear delts, biceps, traps, erectors
color: 🟣 Technical | Tier 2-5 | GOLD: Yes | Precision. Lower volume, extended rest, quality focus.
blocks: ♨️ 🎼 🧈 🧩 🪫 🚂
---
# 🪡 Yoke Walk — Pull Technical 🪡

## Full Body Aesthetic — Pull focus (Technical) · 40-55 min

**CODE:** 🌾🌹🪡🟣

> "Flow through the full body as one integrated pattern."
═══
## 1) ♨️ Warm-Up — 🦢 plico
Subcode: 🌾🌹🪡🟣 (Warm-Up | Pull | Aesthetic | Technical)
├─ 10 🪡 Single-Arm Face Pull (precision, quality over volume)
│  Set 1: 🌾 50% × 10 (pattern prep)
Rest: 60s
═══
## 2) 🎼 Composition
Subcode: 🌾🌹🪡🟣 (Composition | Pull | Aesthetic | Technical)
├─ 9 🪡 Aquaman (Alternating Superman) → Yoke Walk (flow without reset)
│  Set 1: 🌾 70% × 9 (unified pattern)
│  Set 2: 🌾 70% × 9 (repeat flow)
Rest: 60s
═══
## 3) 🧈 Bread & Butter
Subcode: 🌾🌹🪡🟣 (Bread & Butter | Pull | Aesthetic | Technical)
├─ 9 🪡 Yoke Walk (precision, quality over volume)
│  Set 1: 🌾 65% × 9 (build set)
│  Set 2: 🌾 70% × 9 (working set)
│  Set 3: 🌾 70% × 9 (repeat quality)
Rest: 60s
═══
## 4) 🧩 Supplemental
Subcode: 🌾🌹🪡🟣 (Supplemental | Pull | Aesthetic | Technical)
├─ 11 🪡 Wide-Grip Lat Pulldown (Teres Focus) (full range, different angle)
│  Set 1: 🌾 60% × 11 (support volume)
│  Set 2: 🌾 60% × 11 (non-redundant)
Rest: 45s
═══
## 5) 🪫 Release
Subcode: 🌾🌹🪡🟣 (Release | Pull | Aesthetic | Technical)
├─ 12 🪡 Good Morning (Barbell) (smooth tempo, downshift)
│  Set 1: 🌾 50% × 12 (deload)
Rest: 45s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🌾🌹🪡🟣 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
The flow is the measure. If movements disconnected, simplify next session.
