---
zip: 🌾🌹➕🔴
operator: 🦢 plico
status: GENERATED
deck: 27
order: 🌾 Full Body | ~70% | 8-10 reps | 30-90s | CNS: Moderate
axis: 🌹 Aesthetic | Isolation, full ROM, mind-muscle connection. Feel over load.
type: ➕ Plus | Full body power, core
color: 🔴 Intense | Tier 2-4 | GOLD: Yes | Maximum effort. High volume. Reduced rest. Supersets OK.
blocks: ♨️ 🎼 🧈 🧩 🪫 🚂
---
# ➕ Single-Foot In Each Box (Forward) — Plus Intense ➕

## Full Body Aesthetic — Plus focus (Intense) · 40-55 min

**CODE:** 🌾🌹➕🔴

> "Flow through the full body as one integrated pattern."
═══
## 1) ♨️ Warm-Up — 🦢 plico
Subcode: 🌾🌹➕🔴 (Warm-Up | Plus | Aesthetic | Intense)
├─ 10 ➕ Single-Leg Box Jump (high effort, push the pace)
│  Set 1: 🌾 50% × 10 (pattern prep)
Rest: 60s
═══
## 2) 🎼 Composition
Subcode: 🌾🌹➕🔴 (Composition | Plus | Aesthetic | Intense)
├─ 9 ➕ Single-Leg Hops Through Ladder → Single-Foot In Each Box (Forward) (flow without reset)
│  Set 1: 🌾 70% × 9 (unified pattern)
│  Set 2: 🌾 70% × 9 (repeat flow)
Rest: 60s
═══
## 3) 🧈 Bread & Butter
Subcode: 🌾🌹➕🔴 (Bread & Butter | Plus | Aesthetic | Intense)
├─ 9 ➕ Single-Foot In Each Box (Forward) (high effort, push the pace)
│  Set 1: 🌾 65% × 9 (build set)
│  Set 2: 🌾 70% × 9 (working set)
│  Set 3: 🌾 70% × 9 (repeat quality)
Rest: 60s
═══
## 4) 🧩 Supplemental
Subcode: 🌾🌹➕🔴 (Supplemental | Plus | Aesthetic | Intense)
├─ 11 ➕ Sandbag Shouldering (Alternating) (full range, different angle)
│  Set 1: 🌾 60% × 11 (support volume)
│  Set 2: 🌾 60% × 11 (non-redundant)
Rest: 45s
═══
## 5) 🪫 Release
Subcode: 🌾🌹➕🔴 (Release | Plus | Aesthetic | Intense)
├─ 12 ➕ Half-Kneeling Cable Chop (stress out, cathartic discharge)
│  Set 1: 🌾 50% × 12 (deload)
Rest: 45s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🌾🌹➕🔴 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
The flow is the measure. If movements disconnected, simplify next session.
