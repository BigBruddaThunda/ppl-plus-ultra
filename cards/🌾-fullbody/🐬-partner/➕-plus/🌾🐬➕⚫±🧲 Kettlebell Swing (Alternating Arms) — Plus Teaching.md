---
zip: 🌾🐬➕⚫
operator: 🧲 capio
status: GENERATED
deck: 30
order: 🌾 Full Body | ~70% | 8-10 reps | 30-90s | CNS: Moderate
axis: 🐬 Partner | Social training context. Spottable, alternating, teachable.
type: ➕ Plus | Full body power, core
color: ⚫ Teaching | Tier 2-3 | GOLD: No | Extra rest, coaching cues, comprehension over exertion.
blocks: ♨️ 🎼 🧈 🧩 🪫 🚂
---
# ➕ Kettlebell Swing (Alternating Arms) — Plus Teaching ➕

## Full Body Partner — Plus focus (Teaching) · 40-55 min

**CODE:** 🌾🐬➕⚫

> "Flow through the full body as one integrated pattern."
═══
## 1) ♨️ Warm-Up — 🧲 capio
Subcode: 🌾🐬➕⚫ (Warm-Up | Plus | Partner | Teaching)
├─ 10 ➕ Kettlebell Swing (Single-Arm) (coached, check form before adding load)
│  Set 1: 🌾 50% × 10 (pattern prep)
Rest: 60s
═══
## 2) 🎼 Composition
Subcode: 🌾🐬➕⚫ (Composition | Plus | Partner | Teaching)
├─ 9 ➕ Single-Arm Face Pull → Kettlebell Swing (Alternating Arms) (flow without reset)
│  Set 1: 🌾 70% × 9 (unified pattern)
│  Set 2: 🌾 70% × 9 (repeat flow)
Rest: 60s
═══
## 3) 🧈 Bread & Butter
Subcode: 🌾🐬➕⚫ (Bread & Butter | Plus | Partner | Teaching)
├─ 9 ➕ Kettlebell Swing (Alternating Arms) (coached, check form before adding load)
│  Set 1: 🌾 65% × 9 (build set)
│  Set 2: 🌾 70% × 9 (working set)
│  Set 3: 🌾 70% × 9 (repeat quality)
Rest: 60s
═══
## 4) 🧩 Supplemental
Subcode: 🌾🐬➕⚫ (Supplemental | Plus | Partner | Teaching)
├─ 11 ➕ Kettlebell Goblet Squat (full range, different angle)
│  Set 1: 🌾 60% × 11 (support volume)
│  Set 2: 🌾 60% × 11 (non-redundant)
Rest: 45s
═══
## 5) 🪫 Release
Subcode: 🌾🐬➕⚫ (Release | Plus | Partner | Teaching)
├─ 12 ➕ Single-Leg Romanian Deadlift (smooth tempo, downshift)
│  Set 1: 🌾 50% × 12 (deload)
Rest: 45s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🌾🐬➕⚫ — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
The flow is the measure. If movements disconnected, simplify next session.
