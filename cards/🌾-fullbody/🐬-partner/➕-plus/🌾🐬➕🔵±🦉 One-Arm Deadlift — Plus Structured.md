---
zip: 🌾🐬➕🔵
operator: 🦉 logos
status: GENERATED
deck: 30
order: 🌾 Full Body | ~70% | 8-10 reps | 30-90s | CNS: Moderate
axis: 🐬 Partner | Social training context. Spottable, alternating, teachable.
type: ➕ Plus | Full body power, core
color: 🔵 Structured | Tier 2-3 | GOLD: No | Prescribed sets/reps/rest. Trackable and repeatable.
blocks: ♨️ 🎼 🧈 🧩 🪫 🚂
---
# ➕ One-Arm Deadlift — Plus Structured ➕

## Full Body Partner — Plus focus (Structured) · 40-55 min

**CODE:** 🌾🐬➕🔵

> "Prescribed flow. Each movement connects to the next without reset."
═══
## 1) ♨️ Warm-Up — 🦉 logos
Subcode: 🌾🐬➕🔵 (Warm-Up | Plus | Partner | Structured)
├─ 10 ➕ Kettlebell Swing (Single-Arm) (prescribed, track sets and reps)
│  Set 1: 🌾 50% × 10 (pattern prep)
Rest: 60s
═══
## 2) 🎼 Composition
Subcode: 🌾🐬➕🔵 (Composition | Plus | Partner | Structured)
├─ 9 ➕ Kettlebell Goblet Squat → One-Arm Deadlift (flow without reset)
│  Set 1: 🌾 70% × 9 (unified pattern)
│  Set 2: 🌾 70% × 9 (repeat flow)
Rest: 60s
═══
## 3) 🧈 Bread & Butter
Subcode: 🌾🐬➕🔵 (Bread & Butter | Plus | Partner | Structured)
├─ 9 ➕ One-Arm Deadlift (prescribed, track sets and reps)
│  Set 1: 🌾 65% × 9 (build set)
│  Set 2: 🌾 70% × 9 (working set)
│  Set 3: 🌾 70% × 9 (repeat quality)
Rest: 60s
═══
## 4) 🧩 Supplemental
Subcode: 🌾🐬➕🔵 (Supplemental | Plus | Partner | Structured)
├─ 10 ➕ Single-Leg Romanian Deadlift (full range, different angle)
│  Set 1: 🌾 60% × 10 (support volume)
│  Set 2: 🌾 60% × 10 (non-redundant)
Rest: 45s
═══
## 5) 🪫 Release
Subcode: 🌾🐬➕🔵 (Release | Plus | Partner | Structured)
├─ 10 ➕ Half-Kneeling Pallof Press (smooth tempo, downshift)
│  Set 1: 🌾 50% × 10 (deload)
Rest: 45s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🌾🐬➕🔵 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
The flow is the measure. If movements disconnected, simplify next session.
