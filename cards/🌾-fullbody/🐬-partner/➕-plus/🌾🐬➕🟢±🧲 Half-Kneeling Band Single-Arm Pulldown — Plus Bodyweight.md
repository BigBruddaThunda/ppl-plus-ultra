---
zip: 🌾🐬➕🟢
operator: 🧲 capio
status: GENERATED
deck: 30
order: 🌾 Full Body | ~70% | 8-10 reps | 30-90s | CNS: Moderate
axis: 🐬 Partner | Social training context. Spottable, alternating, teachable.
type: ➕ Plus | Full body power, core
color: 🟢 Bodyweight | Tier 0-2 | GOLD: No | No gym required. Park, hotel, living room.
blocks: ♨️ 🎼 🧈 🧩 🪫 🚂
---
# ➕ Hip Flexor Stretch (Half-Kneeling) — Plus Bodyweight ➕

## Full Body Partner — Plus focus (Bodyweight) · 40-55 min

**CODE:** 🌾🐬➕🟢

> "Flow through the body using only what you carry. No equipment needed."

*Youth Athletic Development: Bodyweight only — safe, accessible, no equipment barrier to entry*
═══
## 1) ♨️ Warm-Up — 🧲 capio
Subcode: 🌾🐬➕🟢 (Warm-Up | Plus | Partner | Bodyweight)
├─ 10 ➕ Leg Swings (Forward/Backward) (bodyweight, no external load needed)
│  Set 1: 🌾 50% × 10 (pattern prep)
Rest: 60s
═══
## 2) 🎼 Composition
Subcode: 🌾🐬➕🟢 (Composition | Plus | Partner | Bodyweight)
├─ 9 ➕ Leg Swings (Lateral/Side-to-Side) → Hip Flexor Stretch (Half-Kneeling) (flow without reset)
│  Set 1: 🌾 70% × 9 (unified pattern)
│  Set 2: 🌾 70% × 9 (repeat flow)
Rest: 60s
═══
## 3) 🧈 Bread & Butter
Subcode: 🌾🐬➕🟢 (Bread & Butter | Plus | Partner | Bodyweight)
├─ 9 ➕ Hip Flexor Stretch (Half-Kneeling) (bodyweight, no external load needed)
│  Set 1: 🌾 65% × 9 (build set)
│  Set 2: 🌾 70% × 9 (working set)
│  Set 3: 🌾 70% × 9 (repeat quality)
Rest: 60s
═══
## 4) 🧩 Supplemental
Subcode: 🌾🐬➕🟢 (Supplemental | Plus | Partner | Bodyweight)
├─ 10 ➕ Walking Lunge with Twist (full range, different angle)
│  Set 1: 🌾 60% × 10 (support volume)
│  Set 2: 🌾 60% × 10 (non-redundant)
Rest: 45s
═══
## 5) 🪫 Release
Subcode: 🌾🐬➕🟢 (Release | Plus | Partner | Bodyweight)
├─ 10 ➕ Spiderman Lunge (smooth tempo, downshift)
│  Set 1: 🌾 50% × 10 (deload)
Rest: 45s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🌾🐬➕🟢 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Flow at bodyweight reveals coordination. Log the movement quality.
