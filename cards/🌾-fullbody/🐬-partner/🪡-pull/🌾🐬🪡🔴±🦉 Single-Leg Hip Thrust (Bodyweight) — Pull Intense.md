---
zip: 🌾🐬🪡🔴
operator: 🦉 logos
status: GENERATED
deck: 30
order: 🌾 Full Body | ~70% | 8-10 reps | 30-90s | CNS: Moderate
axis: 🐬 Partner | Social training context. Spottable, alternating, teachable.
type: 🪡 Pull | Lats, rear delts, biceps, traps, erectors
color: 🔴 Intense | Tier 2-4 | GOLD: Yes | Maximum effort. High volume. Reduced rest. Supersets OK.
blocks: ♨️ 🎼 🧈 🧩 🪫 🚂
---
# 🪡 Zercher Carry — Pull Intense 🪡

## Full Body Partner — Pull focus (Intense) · 40-55 min

**CODE:** 🌾🐬🪡🔴

> "Drive through the integrated pattern with intent. No wasted motion."

*Combat Sport Prep: High-output functional work — explosive repeats, full effort, game-speed*
═══
## 1) ♨️ Warm-Up — 🦉 logos
Subcode: 🌾🐬🪡🔴 (Warm-Up | Pull | Partner | Intense)
├─ 10 🪡 Keg Carry (high effort, push the pace)
│  Set 1: 🌾 50% × 10 (pattern prep)
Rest: 30s
═══
## 2) ▶️ Primer
Subcode: 🌾🐬🪡🔴 (Primer | Pull | Partner | Intense)
├─ 9 🪡 Archer Pull-Up (tight setup, crisp intent)
│  Set 1: 🌾 60% × 9 (activation)
Rest: 30s
═══
## 3) 🧈 Bread & Butter
Subcode: 🌾🐬🪡🔴 (Bread & Butter | Pull | Partner | Intense)
├─ 9 🪡 Zercher Carry (high effort, push the pace)
│  Set 1: 🌾 65% × 9 (build set)
│  Set 2: 🌾 70% × 9 (working set)
│  Set 3: 🌾 70% × 9 (repeat quality)
Rest: 30s
═══
## 4) 🧩 Supplemental
Subcode: 🌾🐬🪡🔴 (Supplemental | Pull | Partner | Intense)
├─ 10 🪡 Single-Arm Lat Pulldown (full range, different angle)
│  Set 1: 🌾 60% × 10 (support volume)
│  Set 2: 🌾 60% × 10 (non-redundant)
Rest: 30s
═══
## 5) 🌋 Gutter
Subcode: 🌾🐬🪡🔴 (Gutter | Pull | Partner | Intense)
├─ 10 🪡 Single-Arm Dumbbell Row (all-out, leave nothing in reserve)
│  Set 1: 🌾 55% × 10 (push to form break)
│  Set 2: 🌾 55% × AMRAP (final effort)
Rest: 45s
═══
## 6) 🪫 Release
Subcode: 🌾🐬🪡🔴 (Release | Pull | Partner | Intense)
├─ 10 🪡 Single-Arm Dumbbell Row (stress out, cathartic discharge)
│  Set 1: 🌾 50% × 10 (deload)
Rest: 30s
═══
## 7) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🌾🐬🪡🔴 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Integrated effort under fatigue reveals the real pattern. Log the breakdown point.
