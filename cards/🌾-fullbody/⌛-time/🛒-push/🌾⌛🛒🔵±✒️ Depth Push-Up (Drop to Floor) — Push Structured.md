---
zip: 🌾⌛🛒🔵
operator: ✒️ grapho
status: GENERATED
deck: 29
order: 🌾 Full Body | ~70% | 8-10 reps | 30-90s | CNS: Moderate
axis: ⌛ Time | Clock as training variable. EMOM, AMRAP, density, timed sets.
type: 🛒 Push | Chest, front delts, triceps
color: 🔵 Structured | Tier 2-3 | GOLD: No | Prescribed sets/reps/rest. Trackable and repeatable.
blocks: ♨️ 🎼 🧈 🧩 🪫 🚂
---
# 🛒 Depth Push-Up (Drop to Floor) — Push Structured 🛒

## Full Body Time — Push focus (Structured) · 40-55 min

**CODE:** 🌾⌛🛒🔵

> "Flow through the full body as one integrated pattern."
═══
## 1) ♨️ Warm-Up — ✒️ grapho
Subcode: 🌾⌛🛒🔵 (Warm-Up | Push | Time | Structured)
├─ 10 🛒 Plyometric Push-Up (Hands Leave Ground) (prescribed, track sets and reps)
│  Set 1: 🌾 50% × 10 (pattern prep)
Rest: 60s
═══
## 2) 🎼 Composition
Subcode: 🌾⌛🛒🔵 (Composition | Push | Time | Structured)
├─ 9 🛒 Single-Arm Cable Chest Fly → Depth Push-Up (Drop to Floor) (flow without reset)
│  Set 1: 🌾 70% × 9 (unified pattern)
│  Set 2: 🌾 70% × 9 (repeat flow)
Rest: 60s
═══
## 3) 🧈 Bread & Butter
Subcode: 🌾⌛🛒🔵 (Bread & Butter | Push | Time | Structured)
├─ 9 🛒 Depth Push-Up (Drop to Floor) (prescribed, track sets and reps)
│  Set 1: 🌾 65% × 9 (build set)
│  Set 2: 🌾 70% × 9 (working set)
│  Set 3: 🌾 70% × 9 (repeat quality)
Rest: 60s
═══
## 4) 🧩 Supplemental
Subcode: 🌾⌛🛒🔵 (Supplemental | Push | Time | Structured)
├─ 11 🛒 Single-Arm Incline Dumbbell Press (full range, different angle)
│  Set 1: 🌾 60% × 11 (support volume)
│  Set 2: 🌾 60% × 11 (non-redundant)
Rest: 45s
═══
## 5) 🪫 Release
Subcode: 🌾⌛🛒🔵 (Release | Push | Time | Structured)
├─ 12 🛒 Single-Arm Landmine Press (Chest Focus) (smooth tempo, downshift)
│  Set 1: 🌾 50% × 12 (deload)
Rest: 45s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🌾⌛🛒🔵 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
The flow is the measure. If movements disconnected, simplify next session.
