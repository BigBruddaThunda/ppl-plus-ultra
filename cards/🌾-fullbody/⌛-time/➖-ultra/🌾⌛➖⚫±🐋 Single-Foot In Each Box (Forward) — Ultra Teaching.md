---
zip: 🌾⌛➖⚫
operator: 🐋 duco
status: GENERATED
deck: 29
order: 🌾 Full Body | ~70% | 8-10 reps | 30-90s | CNS: Moderate
axis: ⌛ Time | Clock as training variable. EMOM, AMRAP, density, timed sets.
type: ➖ Ultra | Cardiovascular system
color: ⚫ Teaching | Tier 2-3 | GOLD: No | Extra rest, coaching cues, comprehension over exertion.
blocks: ♨️ 🎼 🧈 🧩 🪫 🚂
---
# ➖ Rowing Machine (Steady State) — Ultra Teaching ➖

## Full Body Time — Ultra focus (Teaching) · 40-55 min

**CODE:** 🌾⌛➖⚫

> "Connect the chain. Coach each transition between movements."

*Return After Break: Teaching session — learn the movement, understand the cues, build confidence*
═══
## 1) ♨️ Warm-Up — 🐋 duco
Subcode: 🌾⌛➖⚫ (Warm-Up | Ultra | Time | Teaching)
├─ 10 ➖ Rowing Intervals (500m) (coached, check form before adding load)
│  Set 1: 🌾 50% × 10 (pattern prep)
Rest: 90s
═══
## 2) 🛠 Craft
Subcode: 🌾⌛➖⚫ (Craft | Ultra | Time | Teaching)
├─ 9 ➖ Rowing Intervals (1000m) (coach: check grip width, elbow angle, foot pressure)
│  Set 1: 🌾 50% × 9 (practice the pattern, not the load)
│  Set 2: 🌾 55% × 9 (same cues, slightly heavier)
Rest: 90s
═══
## 3) 🧈 Bread & Butter
Subcode: 🌾⌛➖⚫ (Bread & Butter | Ultra | Time | Teaching)
├─ 9 ➖ Rowing Machine (Steady State) (coached, check form before adding load)
│  Set 1: 🌾 65% × 9 (build set)
│  Set 2: 🌾 70% × 9 (working set)
│  Set 3: 🌾 70% × 9 (repeat quality)
Rest: 90s
═══
## 4) 🧩 Supplemental
Subcode: 🌾⌛➖⚫ (Supplemental | Ultra | Time | Teaching)
├─ 10 ➖ Rowing Intervals (2000m) (full range, different angle)
│  Set 1: 🌾 60% × 10 (support volume)
│  Set 2: 🌾 60% × 10 (non-redundant)
Rest: 67s
═══
## 5) 🧬 Imprint
Subcode: 🌾⌛➖⚫ (Imprint | Ultra | Time | Teaching)
├─ 10 ➖ Rowing Sprints (250m) (high rep, low load, neural memory)
│  Set 1: 🌾 45% × 10 (lock the pattern)
Rest: 67s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🌾⌛➖⚫ — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Note which transitions felt disconnected. Coach those links next session.
