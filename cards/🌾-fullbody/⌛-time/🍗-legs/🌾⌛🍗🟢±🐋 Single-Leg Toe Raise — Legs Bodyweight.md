---
zip: 🌾⌛🍗🟢
operator: 🐋 duco
status: GENERATED
deck: 29
order: 🌾 Full Body | ~70% | 8-10 reps | 30-90s | CNS: Moderate
axis: ⌛ Time | Clock as training variable. EMOM, AMRAP, density, timed sets.
type: 🍗 Legs | Quads, hamstrings, glutes, calves
color: 🟢 Bodyweight | Tier 0-2 | GOLD: No | No gym required. Park, hotel, living room.
blocks: ♨️ 🎼 🧈 🧩 🪫 🚂
---
# 🍗 Single-Leg Toe Raise — Legs Bodyweight 🍗

## Full Body Time — Legs focus (Bodyweight) · 40-55 min

**CODE:** 🌾⌛🍗🟢

> "Flow through the full body as one integrated pattern."
═══
## 1) ♨️ Warm-Up — 🐋 duco
Subcode: 🌾⌛🍗🟢 (Warm-Up | Legs | Time | Bodyweight)
├─ 10 🍗 Single-Leg Pogo Hops (bodyweight, no external load needed)
│  Set 1: 🌾 50% × 10 (pattern prep)
Rest: 60s
═══
## 2) 🎼 Composition
Subcode: 🌾⌛🍗🟢 (Composition | Legs | Time | Bodyweight)
├─ 9 🍗 Half-Kneeling Hip Flexor Stretch → Single-Leg Toe Raise (flow without reset)
│  Set 1: 🌾 70% × 9 (unified pattern)
│  Set 2: 🌾 70% × 9 (repeat flow)
Rest: 60s
═══
## 3) 🧈 Bread & Butter
Subcode: 🌾⌛🍗🟢 (Bread & Butter | Legs | Time | Bodyweight)
├─ 9 🍗 Single-Leg Toe Raise (bodyweight, no external load needed)
│  Set 1: 🌾 65% × 9 (build set)
│  Set 2: 🌾 70% × 9 (working set)
│  Set 3: 🌾 70% × 9 (repeat quality)
Rest: 60s
═══
## 4) 🧩 Supplemental
Subcode: 🌾⌛🍗🟢 (Supplemental | Legs | Time | Bodyweight)
├─ 11 🍗 Single-Leg Extension (full range, different angle)
│  Set 1: 🌾 60% × 11 (support volume)
│  Set 2: 🌾 60% × 11 (non-redundant)
Rest: 45s
═══
## 5) 🪫 Release
Subcode: 🌾⌛🍗🟢 (Release | Legs | Time | Bodyweight)
├─ 12 🍗 Single-Leg Good Morning (smooth tempo, downshift)
│  Set 1: 🌾 50% × 12 (deload)
Rest: 45s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🌾⌛🍗🟢 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
The flow is the measure. If movements disconnected, simplify next session.
