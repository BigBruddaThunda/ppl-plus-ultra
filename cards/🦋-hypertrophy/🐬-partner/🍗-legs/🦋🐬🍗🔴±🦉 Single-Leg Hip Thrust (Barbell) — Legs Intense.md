---
zip: 🦋🐬🍗🔴
operator: 🦉 logos
status: GENERATED
deck: 18
order: 🦋 Hypertrophy | 65-75% | 8-12 reps | 60-90s | CNS: Moderate
axis: 🐬 Partner | Social training context. Spottable, alternating, teachable.
type: 🍗 Legs | Quads, hamstrings, glutes, calves
color: 🔴 Intense | Tier 2-4 | GOLD: Yes | Maximum effort. High volume. Reduced rest. Supersets OK.
blocks: ♨️ ▶️ 🧈 🗿 🪞/🧩 🪫 🚂
---
# 🍗 Single-Leg Pogo Hops — Legs Intense 🍗

## Hypertrophy Partner — Legs focus (Intense) · 55-70 min

**CODE:** 🦋🐬🍗🔴

> "High volume, low rest. Chase the pump and earn the fatigue."

*Field Sport Athlete: High-output functional work — explosive repeats, full effort, game-speed*
═══
## 1) ♨️ Warm-Up — 🦉 logos
Subcode: 🦋🐬🍗🔴 (Warm-Up | Legs | Partner | Intense)
├─ 12 🍗 Single-Leg Jump Rope (high effort, push the pace)
│  Set 1: 🦋 50% × 12 (pattern prep)
Rest: 30s
═══
## 2) ▶️ Primer
Subcode: 🦋🐬🍗🔴 (Primer | Legs | Partner | Intense)
├─ 10 🍗 Kettlebell Swing (Single-Arm) (tight setup, crisp intent)
│  Set 1: 🦋 60% × 10 (activation)
Rest: 37s
═══
## 3) 🧈 Bread & Butter
Subcode: 🦋🐬🍗🔴 (Bread & Butter | Legs | Partner | Intense)
├─ 10 🍗 Single-Leg Pogo Hops (high effort, push the pace)
│  Set 1: 🦋 65% × 10 (build set)
│  Set 2: 🦋 70% × 10 (working set)
│  Set 3: 🦋 70% × 10 (repeat quality)
Rest: 45s
═══
## 4) 🧩 Supplemental
Subcode: 🦋🐬🍗🔴 (Supplemental | Legs | Partner | Intense)
├─ 12 🍗 Kettlebell Swing (Alternating Arms) (full range, different angle)
│  Set 1: 🦋 60% × 12 (support volume)
│  Set 2: 🦋 60% × 12 (non-redundant)
Rest: 30s
═══
## 5) 🌋 Gutter
Subcode: 🦋🐬🍗🔴 (Gutter | Legs | Partner | Intense)
├─ 12 🍗 Single-Arm American Swing (all-out, leave nothing in reserve)
│  Set 1: 🦋 55% × 12 (push to form break)
│  Set 2: 🦋 55% × AMRAP (final effort)
Rest: 45s
═══
## 6) 🪫 Release
Subcode: 🦋🐬🍗🔴 (Release | Legs | Partner | Intense)
├─ 12 🍗 Single-Arm American Swing (stress out, cathartic discharge)
│  Set 1: 🦋 50% × 12 (deload)
Rest: 30s
═══
## 7) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🦋🐬🍗🔴 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
High volume earned today recovers into growth tomorrow. Eat and sleep.
