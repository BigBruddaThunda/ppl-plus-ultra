---
zip: 🦋⌛➖⚫
operator: 🐋 duco
status: GENERATED
deck: 17
order: 🦋 Hypertrophy | 65-75% | 8-12 reps | 60-90s | CNS: Moderate
axis: ⌛ Time | Clock as training variable. EMOM, AMRAP, density, timed sets.
type: ➖ Ultra | Cardiovascular system
color: ⚫ Teaching | Tier 2-3 | GOLD: No | Extra rest, coaching cues, comprehension over exertion.
blocks: ♨️ ▶️ 🧈 🗿 🪞/🧩 🪫 🚂
---
# ➖ Single-Leg Hops Through Ladder — Ultra Teaching ➖

## Hypertrophy Time — Ultra focus (Teaching) · 55-70 min

**CODE:** 🦋⌛➖⚫

> "Learn what tension feels like in the target muscle before chasing volume."

*Flexibility / Mobility: Teaches what recovery feels like — coaching the nervous system, not the muscles*
═══
## 1) ♨️ Warm-Up — 🐋 duco
Subcode: 🦋⌛➖⚫ (Warm-Up | Ultra | Time | Teaching)
├─ 12 ➖ Sprint (10-30 meters) (coached, check form before adding load)
│  Set 1: 🦋 50% × 12 (pattern prep)
Rest: 90s
═══
## 2) 🛠 Craft
Subcode: 🦋⌛➖⚫ (Craft | Ultra | Time | Teaching)
├─ 10 ➖ Sprint (30-60 meters) (coach: check grip width, elbow angle, foot pressure)
│  Set 1: 🦋 50% × 10 (practice the pattern, not the load)
│  Set 2: 🦋 55% × 10 (same cues, slightly heavier)
Rest: 112s
═══
## 3) 🧈 Bread & Butter
Subcode: 🦋⌛➖⚫ (Bread & Butter | Ultra | Time | Teaching)
├─ 10 ➖ Single-Leg Hops Through Ladder (coached, check form before adding load)
│  Set 1: 🦋 65% × 10 (build set)
│  Set 2: 🦋 70% × 10 (working set)
│  Set 3: 🦋 70% × 10 (repeat quality)
Rest: 135s
═══
## 4) 🧩 Supplemental
Subcode: 🦋⌛➖⚫ (Supplemental | Ultra | Time | Teaching)
├─ 12 ➖ Sprint (60-100 meters) (full range, different angle)
│  Set 1: 🦋 60% × 12 (support volume)
│  Set 2: 🦋 60% × 12 (non-redundant)
Rest: 90s
═══
## 5) 🧬 Imprint
Subcode: 🦋⌛➖⚫ (Imprint | Ultra | Time | Teaching)
├─ 12 ➖ Hill Sprint (Long) (high rep, low load, neural memory)
│  Set 1: 🦋 45% × 12 (lock the pattern)
Rest: 90s
═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🦋⌛➖⚫ — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Track the pump and the tension. Learning to feel the muscle is the skill.
