---
zip: 🦋🪐🍗🟢
operator: 🪵 teneo
status: GENERATED
deck: 16
order: 🦋 Hypertrophy | 65-75% | 8-12 reps | 60-90s | CNS: Moderate
axis: 🪐 Challenge | Hardest variation at any level. Deficit, pause, tempo, bands.
type: 🍗 Legs | Quads, hamstrings, glutes, calves
color: 🟢 Bodyweight | Tier 0-2 | GOLD: No | No gym required. Park, hotel, living room.
blocks: ♨️ ▶️ 🧈 🗿 🪞/🧩 🪫 🚂
---
# 🍗 Single-Leg Jump Rope — Legs Bodyweight 🍗

## Hypertrophy Challenge — Legs focus (Bodyweight) · 55-70 min

**CODE:** 🦋🪐🍗🟢

> "Build the muscle with what you have. No equipment is not an excuse."

*General Functional Fitness: Bodyweight functional — park, field, living room — prove the pattern works anywhere*
═══
## 1) ♨️ Warm-Up — 🪵 teneo
Subcode: 🦋🪐🍗🟢 (Warm-Up | Legs | Challenge | Bodyweight)
├─ 12 🍗 Single-Leg Hip Thrust (Dumbbell) (bodyweight, no external load needed)
│  Set 1: 🦋 50% × 12 (pattern prep)
Rest: 60s
═══
## 2) ▶️ Primer
Subcode: 🦋🪐🍗🟢 (Primer | Legs | Challenge | Bodyweight)
├─ 10 🍗 Single-Leg Hip Thrust (Bodyweight) (tight setup, crisp intent)
│  Set 1: 🦋 60% × 10 (activation)
Rest: 75s
═══
## 3) 🧈 Bread & Butter
Subcode: 🦋🪐🍗🟢 (Bread & Butter | Legs | Challenge | Bodyweight)
├─ 10 🍗 Single-Leg Jump Rope (bodyweight, no external load needed)
│  Set 1: 🦋 65% × 10 (build set)
│  Set 2: 🦋 70% × 10 (working set)
│  Set 3: 🦋 70% × 10 (repeat quality)
Rest: 90s
═══
## 4) 🗿 Sculpt
Subcode: 🦋🪐🍗🟢 (Sculpt | Legs | Challenge | Bodyweight)
├─ 12 🍗 Single-Leg Glute Bridge (angles, tension, volume)
│  Set 1: 🦋 65% × 12 (shaping)
│  Set 2: 🦋 65% × 12 (carving)
Rest: 60s
═══
## 5) 🧩 Supplemental
Subcode: 🦋🪐🍗🟢 (Supplemental | Legs | Challenge | Bodyweight)
├─ 12 🍗 Single-Leg Glute Bridge (full range, different angle)
│  Set 1: 🦋 60% × 12 (support volume)
│  Set 2: 🦋 60% × 12 (non-redundant)
Rest: 60s
═══
## 6) 🪫 Release
Subcode: 🦋🪐🍗🟢 (Release | Legs | Challenge | Bodyweight)
├─ 12 🍗 Single-Leg Balance Hold (smooth tempo, downshift)
│  Set 1: 🦋 50% × 12 (deload)
Rest: 60s
═══
## 7) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🦋🪐🍗🟢 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Bodyweight hypertrophy works when tempo and volume are honest. Log both.
