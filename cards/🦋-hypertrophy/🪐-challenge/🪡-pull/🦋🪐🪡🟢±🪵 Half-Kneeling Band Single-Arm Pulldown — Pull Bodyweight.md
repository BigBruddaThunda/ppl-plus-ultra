---
zip: 🦋🪐🪡🟢
operator: 🪵 teneo
status: GENERATED
deck: 16
order: 🦋 Hypertrophy | 65-75% | 8-12 reps | 60-90s | CNS: Moderate
axis: 🪐 Challenge | Hardest variation at any level. Deficit, pause, tempo, bands.
type: 🪡 Pull | Lats, rear delts, biceps, traps, erectors
color: 🟢 Bodyweight | Tier 0-2 | GOLD: No | No gym required. Park, hotel, living room.
blocks: ♨️ ▶️ 🧈 🗿 🪞/🧩 🪫 🚂
---
# 🪡 Single-Arm Face Pull — Pull Bodyweight 🪡

## Hypertrophy Challenge — Pull focus (Bodyweight) · 55-70 min

**CODE:** 🦋🪐🪡🟢

> "Build the muscle with what you have. No equipment is not an excuse."

*Climbing / Grip Sport: Bodyweight functional — park, field, living room — prove the pattern works anywhere*
═══
## 1) ♨️ Warm-Up — 🪵 teneo
Subcode: 🦋🪐🪡🟢 (Warm-Up | Pull | Challenge | Bodyweight)
├─ 12 🪡 Aquaman (Alternating Superman) (bodyweight, no external load needed)
│  Set 1: 🦋 50% × 12 (pattern prep)
Rest: 60s
═══
## 2) ▶️ Primer
Subcode: 🦋🪐🪡🟢 (Primer | Pull | Challenge | Bodyweight)
├─ 10 🪡 Half-Kneeling Band Single-Arm Pulldown (tight setup, crisp intent)
│  Set 1: 🦋 60% × 10 (activation)
Rest: 75s
═══
## 3) 🧈 Bread & Butter
Subcode: 🦋🪐🪡🟢 (Bread & Butter | Pull | Challenge | Bodyweight)
├─ 10 🪡 Single-Arm Face Pull (bodyweight, no external load needed)
│  Set 1: 🦋 65% × 10 (build set)
│  Set 2: 🦋 70% × 10 (working set)
│  Set 3: 🦋 70% × 10 (repeat quality)
Rest: 90s
═══
## 4) 🗿 Sculpt
Subcode: 🦋🪐🪡🟢 (Sculpt | Pull | Challenge | Bodyweight)
├─ 12 🪡 Archer Pull-Up (angles, tension, volume)
│  Set 1: 🦋 65% × 12 (shaping)
│  Set 2: 🦋 65% × 12 (carving)
Rest: 60s
═══
## 5) 🧩 Supplemental
Subcode: 🦋🪐🪡🟢 (Supplemental | Pull | Challenge | Bodyweight)
├─ 12 🪡 Archer Pull-Up (full range, different angle)
│  Set 1: 🦋 60% × 12 (support volume)
│  Set 2: 🦋 60% × 12 (non-redundant)
Rest: 60s
═══
## 6) 🪫 Release
Subcode: 🦋🪐🪡🟢 (Release | Pull | Challenge | Bodyweight)
├─ 12 🪡 Single-Arm Lat Pulldown (smooth tempo, downshift)
│  Set 1: 🦋 50% × 12 (deload)
Rest: 60s
═══
## 7) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🦋🪐🪡🟢 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Bodyweight hypertrophy works when tempo and volume are honest. Log both.
