---
zip: 🦋🪐🛒⚫
operator: 🪵 teneo
status: GENERATED
deck: 16
order: 🦋 Hypertrophy | 65-75% | 8-12 reps | 60-90s | CNS: Moderate
axis: 🪐 Challenge | Hardest variation at any level. Deficit, pause, tempo, bands.
type: 🛒 Push | Chest, front delts, triceps
color: ⚫ Teaching | Tier 2-3 | GOLD: No | Extra rest, coaching cues, comprehension over exertion.
blocks: ♨️ ▶️ 🧈 🗿 🪞/🧩 🪫 🚂
---
# 🛒 Medicine Ball Slam (Chest Height) — Push Teaching 🛒

## Hypertrophy Challenge — Push focus (Teaching) · 55-70 min

**CODE:** 🦋🪐🛒⚫

> "Build tension through volume. Load serves the muscle, not the ego."
═══
## 1) ♨️ Warm-Up — 🪵 teneo
Subcode: 🦋🪐🛒⚫ (Warm-Up | Push | Challenge | Teaching)
├─ 12 🛒 Medicine Ball Chest Pass (coached, check form before adding load)
│  Set 1: 🦋 50% × 12 (pattern prep)
Rest: 60s
═══
## 2) ▶️ Primer
Subcode: 🦋🪐🛒⚫ (Primer | Push | Challenge | Teaching)
├─ 10 🛒 Dumbbell Chest Fly (tight setup, crisp intent)
│  Set 1: 🦋 60% × 10 (activation)
Rest: 75s
═══
## 3) 🧈 Bread & Butter
Subcode: 🦋🪐🛒⚫ (Bread & Butter | Push | Challenge | Teaching)
├─ 10 🛒 Medicine Ball Slam (Chest Height) (coached, check form before adding load)
│  Set 1: 🦋 65% × 10 (build set)
│  Set 2: 🦋 70% × 10 (working set)
│  Set 3: 🦋 70% × 10 (repeat quality)
Rest: 90s
═══
## 4) 🗿 Sculpt
Subcode: 🦋🪐🛒⚫ (Sculpt | Push | Challenge | Teaching)
├─ 12 🛒 Low-to-High Cable Fly (angles, tension, volume)
│  Set 1: 🦋 65% × 12 (shaping)
│  Set 2: 🦋 65% × 12 (carving)
Rest: 60s
═══
## 5) 🧩 Supplemental
Subcode: 🦋🪐🛒⚫ (Supplemental | Push | Challenge | Teaching)
├─ 12 🛒 Low-to-High Cable Fly (full range, different angle)
│  Set 1: 🦋 60% × 12 (support volume)
│  Set 2: 🦋 60% × 12 (non-redundant)
Rest: 60s
═══
## 6) 🪫 Release
Subcode: 🦋🪐🛒⚫ (Release | Push | Challenge | Teaching)
├─ 12 🛒 Band Pull-Apart (Chest Opener) (smooth tempo, downshift)
│  Set 1: 🦋 50% × 12 (deload)
Rest: 60s
═══
## 7) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🦋🪐🛒⚫ — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Track the pump and the tension. Volume drives growth; form keeps it honest.
