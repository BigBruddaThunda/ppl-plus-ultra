---
zip: 🦋🪐➖🔴
operator: 🚀 mitto
status: GENERATED
deck: 16
order: 🦋 Hypertrophy | 65-75% | 8-12 reps | 60-90s | CNS: Moderate
axis: 🪐 Challenge | Hardest variation at any level. Deficit, pause, tempo, bands.
type: ➖ Ultra | Cardiovascular system
color: 🔴 Intense | Tier 2-4 | GOLD: Yes | Maximum effort. High volume. Reduced rest. Supersets OK.
blocks: ♨️ ▶️ 🧈 🗿 🪞/🧩 🪫 🚂
---
# ➖ Single-Leg Hops Through Ladder — Ultra Intense ➖

## Hypertrophy Challenge — Ultra focus (Intense) · 55-70 min

**CODE:** 🦋🪐➖🔴

> "High volume, low rest. Chase the pump and earn the fatigue."

*Physique / Recomp: Chase the pump — high volume, low rest, supersets allowed, metabolic stress*
═══
## 1) ♨️ Warm-Up — 🚀 mitto
Subcode: 🦋🪐➖🔴 (Warm-Up | Ultra | Challenge | Intense)
├─ 12 ➖ Sprint (10-30 meters) (high effort, push the pace)
│  Set 1: 🦋 50% × 12 (pattern prep)
Rest: 30s
═══
## 2) ▶️ Primer
Subcode: 🦋🪐➖🔴 (Primer | Ultra | Challenge | Intense)
├─ 10 ➖ Sprint (30-60 meters) (tight setup, crisp intent)
│  Set 1: 🦋 60% × 10 (activation)
Rest: 37s
═══
## 3) 🧈 Bread & Butter
Subcode: 🦋🪐➖🔴 (Bread & Butter | Ultra | Challenge | Intense)
├─ 10 ➖ Single-Leg Hops Through Ladder (high effort, push the pace)
│  Set 1: 🦋 65% × 10 (build set)
│  Set 2: 🦋 70% × 10 (working set)
│  Set 3: 🦋 70% × 10 (repeat quality)
Rest: 45s
═══
## 4) 🧩 Supplemental
Subcode: 🦋🪐➖🔴 (Supplemental | Ultra | Challenge | Intense)
├─ 12 ➖ Sprint (60-100 meters) (full range, different angle)
│  Set 1: 🦋 60% × 12 (support volume)
│  Set 2: 🦋 60% × 12 (non-redundant)
Rest: 30s
═══
## 5) 🌋 Gutter
Subcode: 🦋🪐➖🔴 (Gutter | Ultra | Challenge | Intense)
├─ 12 ➖ Hill Sprint (Short) (all-out, leave nothing in reserve)
│  Set 1: 🦋 55% × 12 (push to form break)
│  Set 2: 🦋 55% × AMRAP (final effort)
Rest: 45s
═══
## 6) 🪫 Release
Subcode: 🦋🪐➖🔴 (Release | Ultra | Challenge | Intense)
├─ 12 ➖ Hill Sprint (Short) (stress out, cathartic discharge)
│  Set 1: 🦋 50% × 12 (deload)
Rest: 30s
═══
## 7) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🦋🪐➖🔴 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
High volume earned today recovers into growth tomorrow. Eat and sleep.
