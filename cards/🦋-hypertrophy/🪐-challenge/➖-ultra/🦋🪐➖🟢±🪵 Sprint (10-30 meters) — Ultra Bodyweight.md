---
zip: 🦋🪐➖🟢
operator: 🪵 teneo
status: GENERATED
deck: 16
order: 🦋 Hypertrophy | 65-75% | 8-12 reps | 60-90s | CNS: Moderate
axis: 🪐 Challenge | Hardest variation at any level. Deficit, pause, tempo, bands.
type: ➖ Ultra | Cardiovascular system
color: 🟢 Bodyweight | Tier 0-2 | GOLD: No | No gym required. Park, hotel, living room.
blocks: ♨️ ▶️ 🧈 🗿 🪞/🧩 🪫 🚂
---
# ➖ Sprint (10-30 meters) — Ultra Bodyweight ➖

## Hypertrophy Challenge — Ultra focus (Bodyweight) · 55-70 min

**CODE:** 🦋🪐➖🟢

> "Build tension through volume. Load serves the muscle, not the ego."
═══
## 1) ♨️ Warm-Up — 🪵 teneo
Subcode: 🦋🪐➖🟢 (Warm-Up | Ultra | Challenge | Bodyweight)
├─ 12 ➖ Single-Leg Jump Rope (bodyweight, no external load needed)
│  Set 1: 🦋 50% × 12 (pattern prep)
Rest: 60s
═══
## 2) ▶️ Primer
Subcode: 🦋🪐➖🟢 (Primer | Ultra | Challenge | Bodyweight)
├─ 10 ➖ Kettlebell Goblet Squat (tight setup, crisp intent)
│  Set 1: 🦋 60% × 10 (activation)
Rest: 75s
═══
## 3) 🧈 Bread & Butter
Subcode: 🦋🪐➖🟢 (Bread & Butter | Ultra | Challenge | Bodyweight)
├─ 10 ➖ Sprint (10-30 meters) (bodyweight, no external load needed)
│  Set 1: 🦋 65% × 10 (build set)
│  Set 2: 🦋 70% × 10 (working set)
│  Set 3: 🦋 70% × 10 (repeat quality)
Rest: 90s
═══
## 4) 🗿 Sculpt
Subcode: 🦋🪐➖🟢 (Sculpt | Ultra | Challenge | Bodyweight)
├─ 12 ➖ Sprint (10-30 meters) (angles, tension, volume)
│  Set 1: 🦋 65% × 12 (shaping)
│  Set 2: 🦋 65% × 12 (carving)
Rest: 60s
═══
## 5) 🧩 Supplemental
Subcode: 🦋🪐➖🟢 (Supplemental | Ultra | Challenge | Bodyweight)
├─ 12 ➖ Sprint (10-30 meters) (full range, different angle)
│  Set 1: 🦋 60% × 12 (support volume)
│  Set 2: 🦋 60% × 12 (non-redundant)
Rest: 60s
═══
## 6) 🪫 Release
Subcode: 🦋🪐➖🟢 (Release | Ultra | Challenge | Bodyweight)
├─ 12 ➖ Sprint (30-60 meters) (smooth tempo, downshift)
│  Set 1: 🦋 50% × 12 (deload)
Rest: 60s
═══
## 7) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🦋🪐➖🟢 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Track the pump and the tension. Volume drives growth; form keeps it honest.
