---
zip: 🦋🔨➖🟠
operator: 🥨 tendo
status: GENERATED
deck: 14
order: 🦋 Hypertrophy | 65-75% | 8-12 reps | 60-90s | CNS: Moderate
axis: 🔨 Functional | Unilateral, standing, athletic-transfer movements.
type: ➖ Ultra | Cardiovascular system
color: 🟠 Circuit | Tier 0-3 | GOLD: No | Station-based timed rotation. No barbells. Loop logic required.
blocks: ♨️ ▶️ 🧈 🗿 🪞/🧩 🪫 🚂
---
# ➖ Turkish Get-Up (To Kneeling) — Ultra Circuit ➖

## Hypertrophy Functional — Ultra focus (Circuit) · 55-70 min

**CODE:** 🦋🔨➖🟠

> "Build tension through volume. Load serves the muscle, not the ego."
═══
## 1) ♨️ Warm-Up — 🥨 tendo
Subcode: 🦋🔨➖🟠 (Warm-Up | Ultra | Functional | Circuit)
├─ 12 ➖ Single-Leg Jump Rope (station rotation, keep moving)
│  Set 1: 🦋 50% × 12 (pattern prep)
Rest: 60s
═══
## 2) ▶️ Primer
Subcode: 🦋🔨➖🟠 (Primer | Ultra | Functional | Circuit)
├─ 10 ➖ Kettlebell Goblet Squat (tight setup, crisp intent)
│  Set 1: 🦋 60% × 10 (activation)
Rest: 75s
═══
## 3) 🎱 ARAM — Circuit Loop
Subcode: 🦋🔨➖🟠 (ARAM | Ultra | Functional | Circuit)
┌─ 3 rounds, rotate through stations. No two adjacent stations same muscle group.
│  Station 1: ➖ Turkish Get-Up (To Kneeling) × 10 (station rotation, keep moving)
│  Station 2: ➖ Single-Leg Jump Rope × 10 (station rotation, keep moving)
│  Station 3: ➖ Kettlebell Goblet Squat × 10 (station rotation, keep moving)
│  Station 4: ➖ Turkish Get-Up (Full) × 10 (station rotation, keep moving)
│  Transition: 15s between stations
│  Round rest: 75s
Rest: 90s after final round
═══
## 6) 🪫 Release
Subcode: 🦋🔨➖🟠 (Release | Ultra | Functional | Circuit)
├─ 12 ➖ Half Turkish Get-Up (To Elbow) (smooth tempo, downshift)
│  Set 1: 🦋 50% × 12 (deload)
Rest: 60s
═══
## 7) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🦋🔨➖🟠 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Track the pump and the tension. Volume drives growth; form keeps it honest.
