---
zip: 🦋🔨🪡🔴
operator: 🥨 tendo
status: GENERATED
deck: 14
order: 🦋 Hypertrophy | 65-75% | 8-12 reps | 60-90s | CNS: Moderate
axis: 🔨 Functional | Unilateral, standing, athletic-transfer movements.
type: 🪡 Pull | Lats, rear delts, biceps, traps, erectors
color: 🔴 Intense | Tier 2-4 | GOLD: Yes | Maximum effort. High volume. Reduced rest. Supersets OK.
blocks: ♨️ ▶️ 🧈 🗿 🪞/🧩 🪫 🚂
---
# 🪡 Alternating Dumbbell Front Raise — Pull Intense 🪡

## Hypertrophy Functional — Pull focus (Intense) · 55-70 min

**CODE:** 🦋🔨🪡🔴

> "High volume, low rest. Chase the pump and earn the fatigue."

*Physique / Recomp: Chase the pump — high volume, low rest, supersets allowed, metabolic stress*
═══
## 1) ♨️ Warm-Up — 🥨 tendo
Subcode: 🦋🔨🪡🔴 (Warm-Up | Pull | Functional | Intense)
├─ 12 🪡 Single-Arm Dumbbell Lateral Raise (high effort, push the pace)
│  Set 1: 🦋 50% × 12 (pattern prep)
Rest: 30s
═══
## 2) ▶️ Primer
Subcode: 🦋🔨🪡🔴 (Primer | Pull | Functional | Intense)
├─ 10 🪡 Single-Arm Cable Rear Delt Fly (tight setup, crisp intent)
│  Set 1: 🦋 60% × 10 (activation)
Rest: 37s
═══
## 3) 🧈 Bread & Butter
Subcode: 🦋🔨🪡🔴 (Bread & Butter | Pull | Functional | Intense)
├─ 10 🪡 Alternating Dumbbell Front Raise (high effort, push the pace)
│  Set 1: 🦋 65% × 10 (build set)
│  Set 2: 🦋 70% × 10 (working set)
│  Set 3: 🦋 70% × 10 (repeat quality)
Rest: 45s
═══
## 4) 🧩 Supplemental
Subcode: 🦋🔨🪡🔴 (Supplemental | Pull | Functional | Intense)
├─ 12 🪡 Single-Arm Overhead Dumbbell Hold (full range, different angle)
│  Set 1: 🦋 60% × 12 (support volume)
│  Set 2: 🦋 60% × 12 (non-redundant)
Rest: 30s
═══
## 5) 🌋 Gutter
Subcode: 🦋🔨🪡🔴 (Gutter | Pull | Functional | Intense)
├─ 12 🪡 Single-Arm Face Pull (all-out, leave nothing in reserve)
│  Set 1: 🦋 55% × 12 (push to form break)
│  Set 2: 🦋 55% × AMRAP (final effort)
Rest: 45s
═══
## 6) 🪫 Release
Subcode: 🦋🔨🪡🔴 (Release | Pull | Functional | Intense)
├─ 12 🪡 Single-Arm Face Pull (stress out, cathartic discharge)
│  Set 1: 🦋 50% × 12 (deload)
Rest: 30s
═══
## 7) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🦋🔨🪡🔴 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
High volume earned today recovers into growth tomorrow. Eat and sleep.
