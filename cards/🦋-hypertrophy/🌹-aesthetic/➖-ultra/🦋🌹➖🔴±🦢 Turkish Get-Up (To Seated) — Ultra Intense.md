---
zip: 🦋🌹➖🔴
operator: 🦢 plico
status: GENERATED
deck: 15
order: 🦋 Hypertrophy | 65-75% | 8-12 reps | 60-90s | CNS: Moderate
axis: 🌹 Aesthetic | Isolation, full ROM, mind-muscle connection. Feel over load.
type: ➖ Ultra | Cardiovascular system
color: 🔴 Intense | Tier 2-4 | GOLD: Yes | Maximum effort. High volume. Reduced rest. Supersets OK.
blocks: ♨️ ▶️ 🧈 🗿 🪞/🧩 🪫 🚂
---
# ➖ Turkish Get-Up (Full) — Ultra Intense ➖

## Hypertrophy Aesthetic — Ultra focus (Intense) · 55-70 min

**CODE:** 🦋🌹➖🔴

> "High volume, low rest. Chase the pump and earn the fatigue."

*Bodybuilding: Chase the pump — high volume, low rest, supersets allowed, metabolic stress*
═══
## 1) ♨️ Warm-Up — 🦢 plico
Subcode: 🦋🌹➖🔴 (Warm-Up | Ultra | Aesthetic | Intense)
├─ 12 ➖ Half Turkish Get-Up (To Elbow) (high effort, push the pace)
│  Set 1: 🦋 50% × 12 (pattern prep)
Rest: 30s
═══
## 2) ▶️ Primer
Subcode: 🦋🌹➖🔴 (Primer | Ultra | Aesthetic | Intense)
├─ 10 ➖ Turkish Get-Up (To Seated) (tight setup, crisp intent)
│  Set 1: 🦋 60% × 10 (activation)
Rest: 37s
═══
## 3) 🧈 Bread & Butter
Subcode: 🦋🌹➖🔴 (Bread & Butter | Ultra | Aesthetic | Intense)
├─ 10 ➖ Turkish Get-Up (Full) (high effort, push the pace)
│  Set 1: 🦋 65% × 10 (build set)
│  Set 2: 🦋 70% × 10 (working set)
│  Set 3: 🦋 70% × 10 (repeat quality)
Rest: 45s
═══
## 4) 🧩 Supplemental
Subcode: 🦋🌹➖🔴 (Supplemental | Ultra | Aesthetic | Intense)
├─ 12 ➖ Turkish Get-Up (To Kneeling) (full range, different angle)
│  Set 1: 🦋 60% × 12 (support volume)
│  Set 2: 🦋 60% × 12 (non-redundant)
Rest: 30s
═══
## 5) 🌋 Gutter
Subcode: 🦋🌹➖🔴 (Gutter | Ultra | Aesthetic | Intense)
├─ 12 ➖ Bottoms-Up Turkish Get-Up (all-out, leave nothing in reserve)
│  Set 1: 🦋 55% × 12 (push to form break)
│  Set 2: 🦋 55% × AMRAP (final effort)
Rest: 45s
═══
## 6) 🪫 Release
Subcode: 🦋🌹➖🔴 (Release | Ultra | Aesthetic | Intense)
├─ 12 ➖ Bottoms-Up Turkish Get-Up (stress out, cathartic discharge)
│  Set 1: 🦋 50% × 12 (deload)
Rest: 30s
═══
## 7) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🦋🌹➖🔴 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
High volume earned today recovers into growth tomorrow. Eat and sleep.
