---
zip: 🦋🏛➖🔴
operator: 🤌 facio
status: GENERATED
deck: 13
order: 🦋 Hypertrophy | 65-75% | 8-12 reps | 60-90s | CNS: Moderate
axis: 🏛 Basics | Bilateral, stable, barbell-first classics. Compound over isolation.
type: ➖ Ultra | Cardiovascular system
color: 🔴 Intense | Tier 2-4 | GOLD: Yes | Maximum effort. High volume. Reduced rest. Supersets OK.
blocks: ♨️ ▶️ 🧈 🗿 🪞/🧩 🪫 🚂
---
# ➖ Kettlebell Goblet Squat — Ultra Intense ➖

## Hypertrophy Basics — Ultra focus (Intense) · 45-55 min

**CODE:** 🦋🏛➖🔴

> "Drive clean reps inside the hypertrophy ceiling and make every set repeatable."

═══
## 1) ♨️ Warm-Up — 🤌 facio inline
Subcode: 🦋🏛➖🔴 (Warm-Up | Ultra | Basics | Intense)
├─ 10 ➖ Rowing Machine (Steady State) (steady tempo, easy ramp)
│  Set 1: 🦋 60% × 10 (pattern prep)
Rest: 75s

═══
## 2) ▶️ Primer
Subcode: 🦋🏛➖🔴 (Primer | Ultra | Basics | Intense)
├─ 8 ➖ Kettlebell Goblet Squat (tight setup, crisp intent)
│  Set 1: 🦋 65% × 8 (activation)
Rest: 90s

═══
## 3) 🧈 Bread & Butter
Subcode: 🦋🏛➖🔴 (Bread & Butter | Ultra | Basics | Intense)
├─ 5 ➖ Kettlebell Goblet Squat (clean line, own the bottom)
│  Set 1: 🦋 75% × 5 (build set)
│  Set 2: 🦋 80% × 5 (working set)
│  Set 3: 🦋 80% × 5 (repeat quality)
Rest: 180s

═══
## 4) 🧩 Supplemental
Subcode: 🦋🏛➖🔴 (Supplemental | Ultra | Basics | Intense)
├─ 6 ➖ Rowing Intervals (500m) (full range, no drift)
│  Set 1: 🦋 70% × 6 (support volume)
Rest: 120s

═══
## 5) 🪫 Release
Subcode: 🦋🏛➖🔴 (Release | Ultra | Basics | Intense)
├─ 12 ➖ Rowing Intervals (1000m) (smooth tempo, downshift)
│  Set 1: 🦋 55% × 12 (tension out)
Rest: 90s

═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🦋🏛➖🔴 — repeat and tighten execution under same constraints.

## 🧮 SAVE
Keep the same movement standard next session and only add load if bar path and position stay unchanged.
