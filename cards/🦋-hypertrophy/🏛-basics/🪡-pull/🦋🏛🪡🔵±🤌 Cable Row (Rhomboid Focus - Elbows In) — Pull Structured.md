---
zip: 🦋🏛🪡🔵
operator: 🤌 facio
status: GENERATED
deck: 13
order: 🦋 Hypertrophy | 65-75% | 8-12 reps | 60-90s | CNS: Moderate
axis: 🏛 Basics | Bilateral, stable, barbell-first classics. Compound over isolation.
type: 🪡 Pull | Lats, rear delts, biceps, traps, erectors
color: 🔵 Structured | Tier 2-3 | GOLD: No | Prescribed sets/reps/rest. Trackable and repeatable.
blocks: ♨️ ▶️ 🧈 🗿 🪞/🧩 🪫 🚂
---
# 🪡 Cable Row (Rhomboid Focus - Elbows In) — Pull Structured 🪡

## Hypertrophy Basics — Pull focus (Structured) · 55-70 min

**CODE:** 🦋🏛🪡🔵

> "Hit every set at the prescribed rep count. Volume is the driver."
═══
## 1) ♨️ Warm-Up — 🤌 facio
Subcode: 🦋🏛🪡🔵 (Warm-Up | Pull | Basics | Structured)
├─ 12 🪡 Wide-Grip Pull-Up (Teres Focus) (prescribed, track sets and reps)
│  Set 1: 🦋 50% × 12 (pattern prep)
Rest: 60s
═══
## 2) ▶️ Primer
Subcode: 🦋🏛🪡🔵 (Primer | Pull | Basics | Structured)
├─ 10 🪡 Chest-Supported Row (Elbow Out) (tight setup, crisp intent)
│  Set 1: 🦋 60% × 10 (activation)
Rest: 75s
═══
## 3) 🧈 Bread & Butter
Subcode: 🦋🏛🪡🔵 (Bread & Butter | Pull | Basics | Structured)
├─ 10 🪡 Cable Row (Rhomboid Focus - Elbows In) (prescribed, track sets and reps)
│  Set 1: 🦋 65% × 10 (build set)
│  Set 2: 🦋 70% × 10 (working set)
│  Set 3: 🦋 70% × 10 (repeat quality)
Rest: 90s
═══
## 4) 🗿 Sculpt
Subcode: 🦋🏛🪡🔵 (Sculpt | Pull | Basics | Structured)
├─ 12 🪡 Cable Row (High Elbow) (angles, tension, volume)
│  Set 1: 🦋 65% × 12 (shaping)
│  Set 2: 🦋 65% × 12 (carving)
Rest: 60s
═══
## 5) 🧩 Supplemental
Subcode: 🦋🏛🪡🔵 (Supplemental | Pull | Basics | Structured)
├─ 12 🪡 Cable Row (High Elbow) (full range, different angle)
│  Set 1: 🦋 60% × 12 (support volume)
│  Set 2: 🦋 60% × 12 (non-redundant)
Rest: 60s
═══
## 6) 🪫 Release
Subcode: 🦋🏛🪡🔵 (Release | Pull | Basics | Structured)
├─ 12 🪡 Deadlift (Conventional) (smooth tempo, downshift)
│  Set 1: 🦋 50% × 12 (deload)
Rest: 60s
═══
## 7) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🦋🏛🪡🔵 — continue at this address.
- Next → [adjacent zip] — explore a neighboring room.
## 🧮 SAVE
Volume drives growth; form keeps it honest. Log sets, reps, and load.
