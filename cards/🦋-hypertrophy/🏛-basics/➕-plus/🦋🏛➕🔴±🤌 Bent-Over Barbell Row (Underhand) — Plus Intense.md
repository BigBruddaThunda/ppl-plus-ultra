---
zip: 🦋🏛➕🔴
operator: 🤌 facio
status: GENERATED
deck: 13
order: 🦋 Hypertrophy | 65-75% | 8-12 reps | 60-90s | CNS: Moderate
axis: 🏛 Basics | Bilateral, stable, barbell-first classics. Compound over isolation.
type: ➕ Plus | Full body power, core
color: 🔴 Intense | Tier 2-4 | GOLD: Yes | Maximum effort. High volume. Reduced rest. Supersets OK.
blocks: ♨️ ▶️ 🧈 🗿 🪞/🧩 🪫 🚂
---
# ➕ Bent-Over Barbell Row (Underhand) — Plus Intense ➕

## Hypertrophy Basics — Plus focus (Intense) · 45-55 min

**CODE:** 🦋🏛➕🔴

> "Drive clean reps inside the hypertrophy ceiling and make every set repeatable."

═══
## 1) ♨️ Warm-Up — 🤌 facio inline
Subcode: 🦋🏛➕🔴 (Warm-Up | Plus | Basics | Intense)
├─ 10 ➕ Zercher Carry (steady tempo, easy ramp)
│  Set 1: 🦋 60% × 10 (pattern prep)
Rest: 75s

═══
## 2) ▶️ Primer
Subcode: 🦋🏛➕🔴 (Primer | Plus | Basics | Intense)
├─ 8 ➕ Pull-Up (Overhand Grip) (tight setup, crisp intent)
│  Set 1: 🦋 65% × 8 (activation)
Rest: 90s

═══
## 3) 🧈 Bread & Butter
Subcode: 🦋🏛➕🔴 (Bread & Butter | Plus | Basics | Intense)
├─ 5 ➕ Bent-Over Barbell Row (Underhand) (clean line, own the bottom)
│  Set 1: 🦋 75% × 5 (build set)
│  Set 2: 🦋 80% × 5 (working set)
│  Set 3: 🦋 80% × 5 (repeat quality)
Rest: 180s

═══
## 4) 🧩 Supplemental
Subcode: 🦋🏛➕🔴 (Supplemental | Plus | Basics | Intense)
├─ 6 ➕ Bent-Over Barbell Row (Overhand) (full range, no drift)
│  Set 1: 🦋 70% × 6 (support volume)
Rest: 120s

═══
## 5) 🪫 Release
Subcode: 🦋🏛➕🔴 (Release | Plus | Basics | Intense)
├─ 12 ➕ Rotational Medicine Ball Throw (smooth tempo, downshift)
│  Set 1: 🦋 55% × 12 (tension out)
Rest: 90s

═══
## 6) 🚂 Junction
- Log: load, reps, and form break point.
- Next → 🦋🏛➕🔴 — repeat and tighten execution under same constraints.

## 🧮 SAVE
Keep the same movement standard next session and only add load if bar path and position stay unchanged.
