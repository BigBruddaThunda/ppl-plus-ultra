...
**Prompt 1 — The Researcher.** Hunts facts. Historical events, astronomical data, agricultural observations, current events, calendar context. Runs in any AI with web browsing. Outputs a structured research brief.

**Prompt 2 — The Content Architect.** Surveys the educational content landscape across eight lanes. Determines the Color of the Day. Runs in any AI with web browsing. Outputs an enriched content brief.

**Prompt 3 — The Editor.** Writes the edition. Receives both briefs and the PPL± system context. Selects the 5 Content Rooms by mapping editorial content to zip codes. Runs in Claude Code or temp architect. Outputs the complete Operis edition.

**Prompt 4 — The Builder.** Proofs the edition, generates workout cards for featured zip codes, files the edition, updates the repository. Runs in Claude Code only. Outputs the proofed edition, new cards, and updated repo state.

The pipeline supersedes the earlier description of the "Five Input Layers" by adding a sixth layer and formalizing the generation mechanism.
